...
namespace Db4oUnit.Extensions.Tests
{
    public class FixtureConfigurationTestCase : ITestCase
    {
        ...
        {
            var configuration = new MockFixtureConfiguration
                ();
            fixture.FixtureConfiguration(configuration);
            Assert.IsTrue(fixture.Label().EndsWith(" - " + configuration.GetLabel()),
                "FixtureConfiguration label must be part of Fixture label."
                );
            new TestRunner(new Db4oTestSuiteBuilder(fixture, new[]
            {
                typeof (TestCase1
                    ),
                typeof (TestCase2)
            })).Run(new TestResult());
            configuration.Verify(new[]
            {
                new MethodCall("configure", new[]
                {
                    MethodCall.Conditions.IsA(typeof (TestCase1)), MethodCall
                        .IgnoredArgument
                }),
                new MethodCall("configure", new[]
                {
                    MethodCall.Conditions
                        .IsA(typeof (TestCase1)),
                    MethodCall.IgnoredArgument
                }),
                new MethodCall("configure", new[]
                {
                    MethodCall.Conditions.IsA(typeof (
                        TestCase2)),
                    MethodCall.IgnoredArgument
                }),
                new MethodCall
                    ("configure", new[]
                    {
                        MethodCall.Conditions.IsA(typeof (TestCase2
                            )),
                        MethodCall.IgnoredArgument
                    })
            });
