...
namespace Db4oTool.TA
{
    internal class MethodEmitter
    {
        ...
        {
            var attributes = MethodAttributes.SpecialName | MethodAttributes.Private | MethodAttributes.Virtual;
            var definition = new MethodDefinition(method.DeclaringType.FullName + "." + method.Name, attributes,
                Import(method.ReturnType));
            foreach (var pi in method.GetParameters())
            {
                definition.Parameters.Add(new ParameterDefinition(pi.Name, ParameterAttributes.None,
                    Import(pi.ParameterType)));
            }
