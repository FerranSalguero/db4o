...
namespace Db4oTool.Core
{
    internal class MethodEditor
    {
        ...
        private static void UpdateInstructionReferences(IEnumerable<Instruction> collection, Instruction oldTarget,
            Instruction newTarget)
        {
            foreach (var instr in collection)
            {
                if (instr.OpCode == OpCodes.Switch)
                {
                    var labels = (Instruction[]) instr.Operand;
        ...
        private static void UpdateInstructionReferences(IEnumerable<ExceptionHandler> handlers, Instruction oldTarget,
            Instruction newTarget)
        {
