...
namespace Db4oTool.Core
{
    public class StackAnalyzer
    {
        ...
        public static StackAnalysisResult IsConsumedBy(Predicate<Instruction> predicate, Instruction start,
        ...
        private StackAnalysisResult Analyze()
        {
            ...
            var stackHeight = 1;
            var offset = 0;
            foreach (var current in InstructionsFollowing(_instruction))
            {
                offset++;
        ...
        private static IEnumerable<Instruction> InstructionsFollowing(Instruction start)
        {
            ...
            while (true)
            {
                ...
                {
                    var branchTarget = (Instruction) current.Operand;
        ...
        {
            var methodReference = (MethodReference) methodCall.Operand;
            var methodDefinition = methodReference.Resolve();
            return (methodDefinition.IsStatic || methodDefinition.IsConstructor ? 0 : 1) +
