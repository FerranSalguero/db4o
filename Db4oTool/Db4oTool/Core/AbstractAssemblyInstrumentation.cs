...
namespace Db4oTool.Core
{
    public abstract class AbstractAssemblyInstrumentation : IAssemblyInstrumentation
    {
        ...
        {
            foreach (var module in _context.Assembly.Modules)
            {
        ...
        protected virtual void ProcessTypes(IEnumerable<TypeDefinition> types, Action<TypeDefinition> action)
        {
        ...
        protected virtual void ProcessTypes(IEnumerable<TypeDefinition> types, Predicate<TypeDefinition> filter,
            Action<TypeDefinition> action)
        {
            foreach (var typedef in new List<TypeDefinition>(types))
            {
        ...
        protected void ProcessMethods(IEnumerable methods)
        {
