...
namespace Db4oTool.Core
{
    ...
    {
        public static string InstructionInformationFor(Instruction instruction,
            Collection<Instruction> containingInstructions)
        {
            var sp = FindSequencePointRelatedTo(instruction, containingInstructions);
            return FormatSourceInformation(sp, instruction);
        }

        private static SequencePoint FindSequencePointRelatedTo(Instruction instruction,
            IEnumerable<Instruction> containingInstructions)
        {
            SequencePoint sp = null;
            foreach (var current in containingInstructions)
            {
        ...
        {
            Func<SequencePoint, string> sequencePointFormater =
                delegate(SequencePoint s)
                {
                    return string.Format("{0} ({1}, {2}) (IL instruction: {3} {4})", s.Document.Url, s.StartLine,
                        s.StartColumn, instruction.OpCode, instruction.Operand);
                };
            ...
            var lineContent = ReadSourceLineFor(sp);
            return lineContent != null
                ? string.Format("{0} ({1}, {2}) : {3}", sp.Document.Url, sp.StartLine, sp.StartColumn,
                    lineContent.Substring(sp.StartColumn - 1, sp.EndColumn - sp.StartColumn - 1))
