...
namespace Db4oTool.Tests.Core
{
    public class InstrumentingCFAssemblyTestCase : ITestCase
    {
        ...
        {
            var assemblyPath = CompileAndInstrumentCFAssembly(version);
            AssertInstrumentedCFAssembly(assemblyPath);
        }

        private static void AssertInstrumentedCFAssembly(string assemblyPath)
        {
            var appDomain = AppDomain.CreateDomain("CF_InstrumentationTest");
        ...
        {
            var po = new ProgramOptions();
            po.ProcessArgs(new[] {"-ta", assemblyPath});
        ...
        {
            var cfAssembliesFolder = CompactFrameworkServices.FolderFor(version);
    ...
    internal class InstrumentationAsserter
    {
        ...
        public void Run()
        {
            ...
            Assert.IsTrue(
                typeof (IActivatable).IsAssignableFrom(
                    assembly.GetType("Db4oTool.Tests.Core.Resources.CFInstrumentationSubject")));
        }
    }
}