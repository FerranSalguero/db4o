...
namespace Db4oTool.Tests.Core
{
    partial class StackAnalyzerTestCase
    {
        private static bool IsMethodCallOnList(Instruction candidate)
        {
            ...
            return callee.DeclaringType.Resolve().FullName ==
                   callee.DeclaringType.Module.Import(typeof (List<>)).FullName;
        }
        ...
        {
            var assembly = AssemblyDefinition.ReadAssembly(GetType().Module.Assembly.Location);
        ...
        {
            var castType = method.DeclaringType.Module.Import(typeof (List<int>)).Resolve();
        ...
        {
            var type = methodDefinition.DeclaringType.Module.Import(typeof (List<int>)).Resolve();
        ...
        {
            var typeReference = FindType(testsDeclaringType);

            var sb = new StringBuilder();
            foreach (MethodReference testMethod in typeReference.Resolve().Methods)
            {
                var methodDefinition = testMethod.Resolve();
                if (!methodDefinition.IsPublic) continue;
                if (methodDefinition.IsConstructor) continue;

                try
                {
                    var stackAnalysis = StackAnalyzer.IsConsumedBy(IsMethodCallOnList, FindCast(methodDefinition),
