...
namespace Db4oTool.Tests.Core
{
    public class ILPatternTestCase : ITestCase
    {
        ...
        {
            var sequence = ILPattern.Sequence(OpCodes.Stsfld, OpCodes.Ldsfld);

            var method = CreateTestMethod(TestSequence1);
            var lastInstruction = LastInstruction(method);
            var context = sequence.BackwardsMatch(lastInstruction);
        ...
        public void TestSequenceIsBackwardsMatch()
        {
            ...
            var lastInstruction = CreateTestMethodAndReturnLastInstruction(TestSequence1);
        ...
        {
            var sequence = ILPattern.Sequence(
        ...
        private static MethodDefinition CreateTestMethod(Emitter emitter)
        {
            var type = new TypeReference("", "Test", null);
            var test = new MethodDefinition("Test", MethodAttributes.Public, type);
        ...
        {
            var type = new TypeReference("", "Test", null);
            var blank = new FieldDefinition("Test", FieldAttributes.Public, type);
            il.Emit(OpCodes.Nop);
        ...
        private delegate void Emitter(ILProcessor il);
    }
}