...
{
    internal class TAWarningOnSignedAssemblyInstrumentation : TAOutputListenerTestCaseBase
    {
        ...
        {
            var signKeyPath = GenerateKeyToSign();
            ...
            if (sign)
            {
                CompilationServices.KeyFile.Using(
                    ...
                    delegate { assembly = GenerateAssembly(ResourceName); });
            }
            else
            {
            ...
            var messages = sign ? new[] {"has been signed"} : new string[0];
            InstrumentAndAssert(assembly.MainModule.FullyQualifiedName, sign, messages);
        }

        private static string GenerateKeyToSign()
        {
            var keyPath = Path.Combine(Path.GetTempPath(), "db4otool-test.skn");
            if (!File.Exists(keyPath))
            {
                var psi = new ProcessStartInfo("sn.exe", "-k " + AppendDoubleQuotationMarks(keyPath));
                psi.RedirectStandardOutput = true;
                psi.RedirectStandardError = true;
                psi.UseShellExecute = false;

                var sn = Process.Start(psi);
