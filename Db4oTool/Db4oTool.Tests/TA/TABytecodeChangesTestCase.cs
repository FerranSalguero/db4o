...
{
    internal class TABytecodeChangesTestCase : ITestCase
    {
        public void TestThisFieldPattern()
        {
            var assemblyPath = InstrumentResource();
            var assembly = AssemblyDefinition.ReadAssembly(assemblyPath);
            var method = GetMethodDefinition(assembly, "Subject", "get_Property");
            var expected = @"
System.String Subject::get_Property()
	...
	IL_0012: ret";
        ...
        private static MethodDefinition GetMethodDefinition(AssemblyDefinition assembly, string typeName,
            string methodName)
        {
        ...
        {
            var resourceName = ResourceServices.CompleteResourceName(GetType(), "TABytecodeChangesSubject");
            var path = CompilationServices.EmitAssemblyFromResource(resourceName);
            var output = InstrumentationServices.InstrumentAssembly("-ta", path);
