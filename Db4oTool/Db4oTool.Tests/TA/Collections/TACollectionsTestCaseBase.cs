...
{
    internal abstract class TACollectionsTestCaseBase : TATestCaseBase, ITestLifeCycle
    {
        ...
        {
            var config = base.Configuration(assemblyLocation);
        ...
        internal static void AssertInstruction(Instruction actual, OpCode opCode, MemberReference expectedCtor)
        {
            ...
            var actualCtor = (MethodReference) actual.Operand;
        ...
        {
            var output = InstrumentAndRunInIsolatedAppDomain(action);
            Assert.IsTrue(
        ...
        {
            CompilationServices.ExtraParameters.Using("/d:" + symbolName,
                delegate { InstrumentAssembly(GenerateAssembly(TestResource), true); });
        }
        ...
        {
            var assembly = GenerateAssembly(TestResource);
            var instrumentationOutput = InstrumentAssembly(assembly, true);

            var testDomain = AppDomain.CreateDomain("TACollectionsDomain", AppDomain.CurrentDomain.Evidence,
                AppDomain.CurrentDomain.SetupInformation);

            try
            {
                testDomain.DoCallBack(
                    new IsolatedAppDomainTestRunner(assembly.MainModule.FullyQualifiedName, action).Run);
            }
            finally
            {
                if (testDomain != null)
        ...
        {
            InstrumentAndRunInIsolatedAppDomain(
                new ConstructorInstrumentationAsserter(TestResource, methodName, ReplacementType, argumentTypes)
                    .AssertIt);
        }
        ...
        {
            InstrumentAndRunInIsolatedAppDomain(
                new ConstructorInstrumentationAsserter(TestResource, methodName, OriginalType).AssertIt);
        }
        ...
        protected void AssertFailingCast(string testMethodName)
        {
            ...
            {
                var expected = ExpectedFailingCastMessage();
                Assert.IsTrue(e.Message.Contains(expected),
                    string.Format("Expected: {0}, Actual: {1}", expected, e.Message));
            }
        }

        private string ExpectedFailingCastMessage()
        {
            return string.Format("Casts to {0} are only allowed for property access/method calls",
                OriginalType.ToString().Replace("[", "<").Replace("]", ">"));
        }
    }
    ...
    internal class ConstructorInstrumentationAsserter
    {
        ...
        public ConstructorInstrumentationAsserter(string testTypeName, string methodName, Type type,
        ...
        {
            var current = ReflectionServices.FindInstruction(assembly, _testTypeName, _methodName, OpCodes.Newobj);
            var foundCtor = TACollectionsTestCaseBase.ContructorFor(TACollectionsTestCaseBase.Import(assembly, _type),
                _parameterTypes);
    ...
    internal class CastAsserter
    {
        ...
        public void AssertIt(AssemblyDefinition assembly)
        {
            var current = ReflectionServices.FindInstruction(assembly, _testTypeName, _testMethodName, OpCodes.Castclass);
    ...
    [Serializable]
    internal class IsolatedAppDomainTestRunner
    {
        ...
        public void Run()
        {
            var instrumentedAssembly = AssemblyDefinition.ReadAssembly(_assemblyPath);
