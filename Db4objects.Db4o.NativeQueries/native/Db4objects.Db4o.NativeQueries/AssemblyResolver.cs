...
namespace Db4objects.Db4o.NativeQueries
{
    ...
    {
        private readonly IDictionary<string, AssemblyDefinition> _assemblies =
        ...
        {
            var scope = (AssemblyNameReference) type.Scope;
            var assemblyName = scope.FullName;
            var definition = LookupAssembly(assemblyName);
            if (null == definition)
            {
                var assembly = Assembly.Load(assemblyName);
                var location = assembly.GetType(type.FullName, true).Module.FullyQualifiedName;
