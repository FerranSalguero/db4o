...
namespace Db4objects.Db4o.Tests.Util
{
    public class VersionServices
    {
        ...
        {
            var raf = new RandomAccessFile(testFile, "r");
            var bytes = new byte[1];
            raf.Read(bytes);
            // readByte() doesn't convert to .NET.
            var db4oHeaderVersion = bytes[0];
            raf.Close();
        ...
        public static int SlotHandlerVersion(IExtObjectContainer objectContainer, object
            obj)
        {
            var id = (int) objectContainer.GetID(obj);
            var objectInfo = objectContainer.GetObjectInfo(obj);
            var container = (ObjectContainerBase) objectContainer;
            var trans = container.Transaction;
            var buffer = container.ReadBufferById(trans, id);
            var context = new UnmarshallingContext(trans, (ObjectReference) objectInfo
