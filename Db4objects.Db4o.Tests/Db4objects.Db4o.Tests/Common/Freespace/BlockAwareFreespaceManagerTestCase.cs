...
namespace Db4objects.Db4o.Tests.Common.Freespace
{
    public class BlockAwareFreespaceManagerTestCase : ITestCase
    {
        ...
        {
            var slot = new Slot(1, 15);
            _nonBlocked.Free(slot);
            AssertContains(new[] {_blockConverter.ToBlockedLength(slot)});
        }
        ...
        public virtual void TestAllocateSlot()
        {
            var slot = new Slot(1, 15);
            _nonBlocked.Free(slot);
            var allocatedSlot = _nonBlocked.AllocateSlot(8);
            var expectedAllocatedSlotLength = _blockConverter.BlockAlignedBytes(8);
            var expectedSlot = new Slot(1, expectedAllocatedSlotLength);
            Assert.AreEqual(expectedSlot, allocatedSlot);
        }

        public virtual void TestNoSlotAvailable()
        {
            var slot = _nonBlocked.AllocateSlot(1);
            Assert.IsNull(slot);
        }
