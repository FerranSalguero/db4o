    ...
    public class FreespaceManagerTestCase : FreespaceManagerTestCaseBase
    {
        ...
        public virtual void TestAllocateTransactionLogSlot()
        {
            for (var i = 0; i < fm.Length; i++)
            {
                ...
                {
                    var slot = fm[i].AllocateTransactionLogSlot(1);
                    Assert.IsNull(slot);
        ...
        {
            for (var from = 0; from < fm.Length; from++)
            {
                for (var to = 0; to < fm.Length; to++)
                {
                    if (to != from)
                    {
                        Clear(fm[from]);
