...
namespace Db4objects.Db4o.Tests.Common.Freespace
{
    public abstract class FreespaceManagerTestCaseBase : FileSizeTestCaseBase, IOptOutMultiSession
    {
        ...
        {
            var container = (LocalObjectContainer) Db();
            var btreeFm = new BTreeFreespaceManager(container, null, container
                .ConfigImpl.DiscardFreeSpace(), AbstractFreespaceManager.RemainderSizeLimit);
            btreeFm.Start(0);
            fm = new IFreespaceManager[]
            {
                new InMemoryFreespaceManager(null, container.ConfigImpl
                    .DiscardFreeSpace(), AbstractFreespaceManager.RemainderSizeLimit),
                btreeFm
            };
        }
