...
namespace Db4objects.Db4o.Tests.Common.Freespace
{
    public class FreespaceRemainderLimitTestCase : ITestCase
    {
        ...
        {
            var slot = new Slot(1, freeSlotSize);
            _nonBlocked.Free(slot);
            var allocatedSlot = _nonBlocked.AllocateSlot(requiredSlotSize);
            var expectedAllocatedSlotLength = _blockConverter.BlockAlignedBytes(expectedSlotSize
                );
