...
namespace Db4objects.Db4o.Tests.Common.Freespace
{
    public class FreespaceManagerTypeChangeSlotCountTestCase : TestWithTempFile
    {
        ...
        public virtual void TestMigrateFromRamToBTree()
        {
            ...
            Reopen();
            CreateFreeSpace();
            var initialSlots = GetSlots(_container.FreespaceManager());
            Reopen();
            var currentSlots = GetSlots(_container.FreespaceManager());
        ...
        private void Open()
        {
            var config = ((IConfiguration) _currentConfig.Run());
        ...
        {
            var slot = _container.AllocateSlot(Size);
