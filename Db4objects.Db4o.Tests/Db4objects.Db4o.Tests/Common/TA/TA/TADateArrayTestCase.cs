...
namespace Db4objects.Db4o.Tests.Common.TA.TA
{
    ...
    {
        public static readonly DateTime[] data =
        {
            new DateTime(0), new DateTime
                (1),
            new DateTime(1191972104500L)
        };
        ...
        {
            var item = new TADateArrayItem();
            item._typed = new DateTime[data.Length];
            item._untyped = new object[data.Length];
            Array.Copy(data, 0, item._typed, 0, data.Length);
            Array.Copy(data, 0, item._untyped, 0, data.Length);
            return item;
        }
    }
}