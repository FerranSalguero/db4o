...
namespace Db4objects.Db4o.Tests.Common.Internal
{
    public class TransactionLocalTestCase : AbstractInMemoryDb4oTestCase
    {
        ...
        {
            var itemBeforeCommit = ItemFor(t1);
            t1.Commit();
            var itemAfterCommit = ItemFor(t1);
            Assert.AreNotSame(itemAfterCommit, itemBeforeCommit);
        }

        public virtual void TestValuesAreDisposedOfOnRollback()
        {
            var itemBeforeRollback = ItemFor(t1);
            t1.Rollback();
            var itemAfterRollback = ItemFor(t1);
            Assert.AreNotSame(itemAfterRollback, itemBeforeRollback);
        }

        private Item ItemFor(Transaction transaction)
        {
            return ((Item) transaction.Get(_subject).value);
        }
