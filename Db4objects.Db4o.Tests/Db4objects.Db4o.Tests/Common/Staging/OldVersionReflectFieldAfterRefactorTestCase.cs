...
namespace Db4objects.Db4o.Tests.Common.Staging
{
    public class OldVersionReflectFieldAfterRefactorTestCase : AbstractDb4oTestCase
    {
        ...
        {
            Store(new ItemBefore(IdValue));
            Reopen();
            FileSession().StoredClass(typeof (ItemBefore
                )).Rename(typeof (ItemAfter).FullName
                );
            Reopen();
            var classMetadata = Container().ClassMetadataForName(typeof (ItemAfter
                ).FullName);
            var originalField = new ByRef();
