...
namespace Db4objects.Db4o.Tests.Common.Soda.Experiments
{
    public class STMagicTestCase : SodaBaseTestCase, ISTInterface
    {
        ...
        public virtual void TestMultiClass()
        {
            ...
            var stDoubles = new STDoubleTestCase().CreateData();
            var stStrings = new STStringTestCase().CreateData();
            var res = new object[stDoubles.Length + stStrings.Length];
            Array.Copy(stDoubles, 0, res, 0, stDoubles.Length);
            Array.Copy(stStrings, 0, res, stDoubles.Length, stStrings.Length);
        ...
        private sealed class _IEvaluation_117 : IEvaluation
        {
            ...
            {
                var sti = (ISTInterface) candidate.GetObject();
