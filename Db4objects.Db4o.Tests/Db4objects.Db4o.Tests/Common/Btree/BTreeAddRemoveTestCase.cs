...
namespace Db4objects.Db4o.Tests.Common.Btree
{
    public class BTreeAddRemoveTestCase : BTreeTestCaseBase
    {
        ...
        {
            var count = BtreeNodeSize + 1;
            for (var i = 0; i < count; i++)
            {
                Add(count + i + 1);
            }
            var smallest = count + 1;
            var trans = NewTransaction();
            for (var i = 0; i < count; i++)
            {
                Add(trans, i);
            }
            var firstPointer = _btree.FirstPointer(Trans());
        ...
        public virtual void TestSearchingRemoved()
        {
            ...
            var range = result.Greater();
            BTreeAssert.AssertRange(new[] {7, 9}, range);
        }
        ...
        public virtual void TestMultiTransactionSearch()
        {
            ...
            Add(Trans(), keys);
            Commit(Trans());
            int[] assorted = {1, 2, 11, 13, 21, 52, 51, 66, 89, 10};
            Add(SystemTrans(), assorted);
            AssertKeys(keys);
            Remove(SystemTrans(), assorted);
            AssertKeys(keys);
            BTreeAssert.AssertRange(new[] {7, 9}, Search(Trans(), 4).Greater());
        }
