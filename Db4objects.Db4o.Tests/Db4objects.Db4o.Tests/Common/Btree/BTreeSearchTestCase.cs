...
namespace Db4objects.Db4o.Tests.Common.Btree
{
    public class BTreeSearchTestCase : AbstractDb4oTestCase, IOptOutDefragSolo, IOptOutMultiSession
    {
        ...
        {
            CycleIntKeys(new[] {3, 5, 7, 10, 11, 12, 14, 15, 17, 20, 21, 25});
        }

        /// <exception cref="System.Exception"></exception>
        private void CycleIntKeys(int[] values)
        {
            var btree = BTreeAssert.CreateIntKeyBTree(Container(), 0, BtreeNodeSize);
            for (var i = 0; i < 5; i++)
            {
        ...
        private void ExpectKeysSearch(Transaction trans, BTree btree, int[] keys)
        {
            ...
            for (var i = 0; i < keys.Length; i++)
            {
                if (keys[i] != lastValue)
                {
                    var expectingVisitor = ExpectingVisitor.CreateExpectingVisitor(keys[
                        i], IntArrays4.Occurences(keys, keys[i]));
                    var range = btree.SearchRange(trans, keys[i]);
