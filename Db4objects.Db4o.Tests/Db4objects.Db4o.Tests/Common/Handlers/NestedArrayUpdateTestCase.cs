...
namespace Db4objects.Db4o.Tests.Common.Handlers
{
    ...
    {
        private static readonly object[] primitiveArrayData =
        {
            new[]
            {
                1, 2, 3
        ...
        private static readonly object[] stringArrayData =
        {
            new[]
            {
                "one", null, string.Empty
            },
            new[] {"two"}, new[]
            {
                string.Empty
        ...
        private static readonly object[] nestedArrayData =
        {
            new object[]
            {
                primitiveArrayData, stringArrayData
            },
            new object[]
            {
                primitiveArrayData, stringArrayData
        ...
        private static readonly object[] nestedNestedArrayData =
        {
            new object
                [] {nestedArrayData, nestedArrayData},
            new object[]
            {
                nestedArrayData, nestedArrayData
            }
        ...
        private void AssertNestedArray(object expected, object actual)
        {
            ...
            {
                var expectedSubArray = (object[]) expectedArray[i];
                var actualSubArray = actualArray[i];
                var template = expectedSubArray[0];
                if (template is int[])
                {
        ...
        private void AssertStringArray(object array)
        {
            ...
            {
                var actual = (string[]) stringArray[i];
                var expected = (string[]) stringArrayData[i];
        ...
        {
            var primitiveArray = (object[]) array;
            for (var i = 0; i < primitiveArray.Length; i++)
            {
                var expected = (int[]) primitiveArrayData[i];
                var actual = CastToIntArray(primitiveArray[i]);
                Assert.AreEqual(actual.Length, expected.Length);
                for (var j = 0; j < expected.Length; j++)
                {
                    Assert.AreEqual(expected[j], actual[j]);
