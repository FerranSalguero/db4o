...
namespace Db4objects.Db4o.Tests.Common.Handlers
{
    public partial class Pre7_1ObjectContainerAdapter : AbstractObjectContainerAdapter
    {
        ...
        private MethodInfo setWithDepthMethod;
        ...
        private void StoreObject(object obj)
        {
            try
            {
                StoreInternal(ResolveSetMethod(), new[] {obj});
            }
            catch (Exception e)
            {
        ...
        private void StoreObject(object obj, int depth)
        {
            try
            {
                StoreInternal(ResolveSetWithDepthMethod(), new[] {obj, depth});
            }
            catch (Exception e)
            {
                Assert.Fail("Call to set method failed.", e);
        ...
        private MethodInfo ResolveSetWithDepthMethod()
        {
            ...
            setWithDepthMethod = db.GetType().GetMethod(SetMethodName(), new[]
            {
                typeof (
                    object),
                typeof (int)
        ...
        private MethodInfo ResolveSetMethod()
        {
            ...
            setMethod = db.GetType().GetMethod(SetMethodName(), new[] {typeof (object)}
                );
            return setMethod;
        }
    }
