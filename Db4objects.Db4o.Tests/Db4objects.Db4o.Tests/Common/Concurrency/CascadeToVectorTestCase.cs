...
namespace Db4objects.Db4o.Tests.Common.Concurrency
{
    public class CascadeToVectorTestCase : Db4oClientServerTestCase
    {
        ...
        {
            var ctv = new CascadeToVectorTestCase();
        ...
        {
            var ctv = (CascadeToVectorTestCase)
                RetrieveOnlyInstance(oc, typeof (CascadeToVectorTestCase));
            var i = ctv.vec.GetEnumerator();
            while (i.MoveNext())
            {
                var atom = (Atom) i.Current;
                atom.name = "updated";
                if (atom.child != null)
                {
        ...
        {
            var os = oc.Query(typeof (CascadeToVectorTestCase));
            if (os.Count == 0)
            {
                // already deleted
                return;
            ...
            var ctv = (CascadeToVectorTestCase) os.Next();
