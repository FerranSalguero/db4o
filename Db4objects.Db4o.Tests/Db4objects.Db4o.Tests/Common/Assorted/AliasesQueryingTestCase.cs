...
namespace Db4objects.Db4o.Tests.Common.Assorted
{
    public class AliasesQueryingTestCase : TestWithTempFile
    {
        ...
        {
            CreateData();
            Querydb(AliasConfig(), new[] {1, 1}, typeof (B), typeof (
                A));
        }
        ...
        {
            CreateData();
            Querydb(AliasConfig(), new[] {1, 1}, typeof (A), typeof (
                B));
        }
        ...
        {
            CreateData();
            var database = Db4oEmbedded.OpenFile(AliasConfig(), TempFile
                ());
            database.Query(typeof (B));
            database.Query(typeof (A));
            database.Close();
            Querydb(Db4oEmbedded.NewConfiguration(), new[] {1, 0}, typeof (A
                ), typeof (B));
        }
        ...
        private IEmbeddedConfiguration AliasConfig()
        {
            ...
            configuration.Common.AddAlias(new TypeAlias(CrossPlatformServices.FullyQualifiedName
                (typeof (A)), CrossPlatformServices.FullyQualifiedName(typeof (
                    B))));
