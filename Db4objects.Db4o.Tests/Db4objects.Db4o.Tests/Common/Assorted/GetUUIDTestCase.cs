...
namespace Db4objects.Db4o.Tests.Common.Assorted
{
    public class GetUUIDTestCase : AbstractDb4oTestCase
    {
        ...
        {
            var itemUUID = GetItemUUID();
            ServerEventRegistry().Committed += new _IEventListener4_34(itemUUID).OnEvent;
        ...
        {
            return Db().Ext().GetObjectInfo(((Item) RetrieveOnlyInstance(typeof (
                Item))));
        }

        public virtual void TestGetUUIDInCommittingCallbacks()
        {
            ServerEventRegistry().Committing += new _IEventListener4_59().OnEvent;
            DeleteAll(typeof (Item));
            Db().Commit();
        }
