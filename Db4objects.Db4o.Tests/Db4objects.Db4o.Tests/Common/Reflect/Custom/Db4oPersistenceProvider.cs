...
namespace Db4objects.Db4o.Tests.Common.Reflect.Custom
{
    ...
    public class Db4oPersistenceProvider : IPersistenceProvider
    {
        ...
        {
            var field = CustomClass(context, className).CustomField(fieldName);
        ...
        {
            var fields = cc.CustomFields();
            while (fields.MoveNext())
            {
                var field = (CustomField) fields.Current;
        ...
        {
            var found = container.Query(typeof (CustomClassRepository));
            if (!found.HasNext())
            {
                return null;
            }
