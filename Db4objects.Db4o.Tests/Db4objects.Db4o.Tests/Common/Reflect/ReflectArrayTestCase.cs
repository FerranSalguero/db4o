...
namespace Db4objects.Db4o.Tests.Common.Reflect
{
    public class ReflectArrayTestCase : AbstractDb4oTestCase
    {
        ...
        {
            return (string[][]) NewInstance(typeof (string), new[] {x, y});
        }

        public virtual void TestIsNDimensional()
        {
            var arrayOfArrayOfString = ReflectClass(typeof (string[][]));
            Assert.IsTrue(arrayOfArrayOfString.IsArray());
            var arrayOfString = ReflectClass(typeof (string[]));
        ...
        {
            var array = NewStringMatrix(3, 4);
            ArrayAssert.AreEqual(new[] {3, 4}, ArrayReflector().Dimensions(array));
        }
