...
namespace Db4objects.Db4o.Tests.Common.Foundation
{
    ...
    public class SortedCollection4TestCase : ITestCase
    {
        ...
        {
            var array = IntArrays4.ToObjectArray(new[] {6, 4, 1, 2, 7, 3});
            var collection = NewSortedCollection();
            Assert.AreEqual(0, collection.Size());
            collection.AddAll(new ArrayIterator4(array));
            AssertCollection(new[] {1, 2, 3, 4, 6, 7}, collection);
        }
        ...
        public virtual void TestAddRemove()
        {
            ...
            collection.Add(1);
            collection.Add(5);
            AssertCollection(new[] {1, 3, 5}, collection);
            collection.Remove(3);
            AssertCollection(new[] {1, 5}, collection);
            collection.Remove(1);
            AssertCollection(new[] {5}, collection);
        }
