...
namespace Db4objects.Db4o.Tests.Common.Types.Arrays
{
    public class ByteArrayTestCase : AbstractDb4oTestCase
    {
        ...
        {
            config.ObjectClass(typeof (SerializableByteArrayHolder)).Translate
                (new TSerializable());
        }
        ...
        {
            TimeQueryLoop("raw byte array", typeof (ByteArrayHolder));
        }
#endif // !CF && !SILVERLIGHT
#if !CF && !SILVERLIGHT
        /// <exception cref="System.Exception"></exception>
        ...
        {
            TimeQueryLoop("TSerializable", typeof (SerializableByteArrayHolder
                ));
        }
#endif // !CF && !SILVERLIGHT
        ...
        private void TimeQueryLoop(string label, Type clazz)
        {
            ...
            while (os.HasNext())
            {
                Assert.AreEqual(ArrayLength, ((IIByteArrayHolder) os.Next()).GetBytes
                    ().Length, label);
            }
        }
