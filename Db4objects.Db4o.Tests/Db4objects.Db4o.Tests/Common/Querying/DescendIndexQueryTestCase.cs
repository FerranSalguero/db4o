...
namespace Db4objects.Db4o.Tests.Common.Querying
{
    public class DescendIndexQueryTestCase : AbstractDb4oTestCase
    {
        ...
        {
            var parentObjectClass = config.ObjectClass(typeof (Parent
                ));
            parentObjectClass.ObjectField("_child").Indexed(true);
            parentObjectClass.ObjectField("_name").Indexed(true);
            var childObjectClass = config.ObjectClass(typeof (Child
                ));
        ...
        public virtual void TestOrChildId()
        {
            ...
            var qChild = q.Descend("_child");
            var qId = qChild.Descend("_id");
            var c1 = qId.Constrain(0);
            var c2 = qId.Constrain(10);
            var c3 = qId.Constrain(100);
            c1.Or(c2).Or(c3);
