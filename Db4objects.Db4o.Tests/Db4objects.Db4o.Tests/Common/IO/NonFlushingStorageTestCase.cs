...
namespace Db4objects.Db4o.Tests.Common.IO
{
    public class NonFlushingStorageTestCase : ITestCase
    {
        ...
        {
            var mock = new MockBin();
            var binConfig = new BinConfiguration("uri", true, 42L, false);
            var storage = new NonFlushingStorage(new _IStorage_19(mock)).Open(binConfig);
            ...
            mock.Verify(new[]
            {
                new MethodCall("open", new object[] {binConfig})
                , new MethodCall("read", new object[] {1L, buffer, 4}), new MethodCall("write"
                    , new object[] {2L, buffer, 3}),
                new MethodCall("length", new object[] {}),
