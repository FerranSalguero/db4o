...
namespace Db4objects.Db4o.Tests.Optional.Monitoring.CS
{
    public class CountingSocket4Factory : ISocket4Factory
    {
        ...
        private readonly IList _sockets = new ArrayList();
        ...
        {
            var socket = new CountingSocket4(_socketFactory.CreateSocket(hostName
                , port));
            _sockets.Add(socket);
            return socket;
        }
        ...
        {
            for (var socketIter = ConnectedClients().GetEnumerator();
                socketIter.MoveNext
                    ();)
            {
                var socket = ((CountingSocket4) socketIter.Current);
                socket.ResetCount();
            }
        }
    }
