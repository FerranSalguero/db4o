...
namespace Db4objects.Db4o.Tests.Jre5.Collections.Typehandler
{
    public abstract class CollectionTypeHandlerUnitTest : TypeHandlerTestUnitBase
    {
            ...
            )
        {
            ...
            var constraint = q.Descend(ItemFactory().FieldName()).Constrain(element);
            if (withContains)
            {
                constraint.Contains();
            }
        ...
        {
            var item = RetrieveItemInstance();
            AssertContent(item);
        }

        protected virtual object RetrieveItemInstance()
        {
            var itemClass = ItemFactory().ItemClass();
            var item = RetrieveOnlyInstance(itemClass);
            return item;
        }
        ...
        public virtual void TestDeletion()
        {
            ...
            var item = RetrieveOnlyInstance(ItemFactory().ItemClass());
            Db().Delete(item);
            Db().Purge();
        ...
        {
            var q = NewQuery(ItemFactory().ItemClass());
            var qq = q.Descend(ItemFactory().FieldName());
