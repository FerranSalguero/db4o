...
{
    internal class CrossDomainRef<T>
#if !SILVERLIGHT
        : MarshalByRefObject
#endif
    {
    ...
    internal class MethodInvoker
    {
        ...
        private readonly CrossDomainRef<object> _returnValue;
        private readonly string _typeName;

        public MethodInvoker(CrossDomainRef<object> returnValue, string typeName, string methodName,
        ...
        public void InvokeInstanceMethod()
        {
            ...
            _returnValue.Value = type.InvokeMember(_methodName,
                BindingFlags.InvokeMethod | BindingFlags.IgnoreCase | BindingFlags.Instance | BindingFlags.Public, null,
                Activator.CreateInstance(type), _arguments);
        }
        ...
        public void InvokeStaticMethod()
        {
            var type = Type.GetType(_typeName, true);
            _returnValue.Value = type.InvokeMember(_methodName,
                BindingFlags.InvokeMethod | BindingFlags.IgnoreCase | BindingFlags.Static | BindingFlags.Public, null,
                null, _arguments);
        }
    }

    [Serializable]
    internal class InstallAssemblyResolver
    {
    ...
    public class Db4oLibraryEnvironment : IDisposable
    {
        ...
        {
#if !CF && !SILVERLIGHT
            var returnValue = new CrossDomainRef<object>();
            _domain.DoCallBack(
                new MethodInvoker(returnValue, ReflectPlatform.FullyQualifiedName(typeof (Db4oFactory)), "Version")
                    .InvokeStaticMethod);
        ...
        public object InvokeInstanceMethod(Type type, string methodName, params object[] args)
        {
            ...
            var returnValue = new CrossDomainRef<object>();
            _domain.DoCallBack(
                new MethodInvoker(returnValue, ReflectPlatform.FullyQualifiedName(type), methodName, args)
        ...
        private void SetUpLegacyAdapter()
        {
            ...
            var adapterAssembly = Path.Combine(_baseDirectory, "Db4objects.Db4o.dll");
        ...
        private void CopyAssemblies(string domainBase)
        {
            ...
            CleanStrongName(IOServices.CopyEnclosingAssemblyTo(typeof (ITest), domainBase));
            CleanStrongName(IOServices.CopyEnclosingAssemblyTo(typeof (IDb4oTestCase), domainBase));
        }
