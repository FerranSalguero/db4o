...
namespace Db4objects.Db4o.Tests.Common.Migration
{
    public class Db4oLibrarian
    {
        ...
        {
#if !SILVERLIGHT
            var libraries = new List<Db4oLibrary>();
            foreach (var path in Directory.GetDirectories(LibraryPath()))
            {
                // comment out the next line to run against legacy versions
                if (!IsVersionOrGreater(path, MinimumVersionToTest))
                    continue;

                var db4oLib = FindLibraryFile(path);
        ...
        private static bool IsVersionOrGreater(string path, string minimumVersion)
        {
            ...
            var candidate = new Version(folderName);
        ...
        {
#if !SILVERLIGHT
            var found = Directory.GetFiles(directory, "*.dll");
