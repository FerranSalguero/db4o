...
namespace Db4objects.Db4o.Tests.NativeQueries.Diagnostics
{
    public partial class NativeQueryOptimizerDiagnosticsTestCase : IOptOutSilverlight
    {
        ...
        {
            _failed = false;
            var items = Db().Query(delegate(Subject sub) { return sub.ComplexName().StartsWith("Test"); });
...
internal class ListenForNQOptimizerLoadFailures : IDiagnosticListener
{
    ...
    public bool UnableToLoadNQOptimizer
    {
        get { return unableToLoadNQOptimizer; }
            ...
            var typeName = "TestNQOptimizerLoadFailure";
            var appDomain = PrepareTestEnvironment(BuildAssemblyFileName(typeName), code);
            try
            {
                var test = (INQTestRunner) appDomain.CreateInstanceAndUnwrap(BuildAssemblyFileName(typeName), typeName);
        ...
        private static AppDomain CreateAppDomain()
        {
            ...
            var newAppDomain = AppDomain.CreateDomain("Db4oNQTest", null, appDomainSetup);

            CopyAssembliesTo(newAppDomain, "Db4oUnit.Extensions.dll", "Db4objects.Db4o.dll", "Db4objects.Db4o.Tests.exe",
                "Db4oUnit.dll");
        ...
        {
            var targetPath = CreateDirectoryIfNecessary(domain.BaseDirectory);
            foreach (var file in toBeCopied)
            {
                File.Copy(
