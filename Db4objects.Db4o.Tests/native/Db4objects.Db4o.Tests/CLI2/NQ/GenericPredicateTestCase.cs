...
namespace Db4objects.Db4o.Tests.CLI2.NQ
{
    ...
    {
        private readonly DiagnosticCollector<NativeQueryNotOptimized> _collector =
            new DiagnosticCollector<NativeQueryNotOptimized>();
        ...
        public void TestGenericClass()
        {
            AssertQueryIsOptimized(
                delegate { new GenericClassHelper<Item>().RunQuery(Db()); }
                );
        }
        ...
        {
            AssertQueryIsOptimized(
                delegate { new GenericMethodHelper().RunQuery<Item>(Db()); }
                );
        }
        ...
        public void TestPredicateContainingBoxIntructionAreOptimized()
        {
            AssertQueryIsOptimized(
                ...
                {
                    Db().Query(
                        delegate(Item candidate) { return candidate.id == 1; });
                }
                );
        }
