...
namespace Db4objects.Db4o.Tests.CLI2.Handlers
{
    ...
    public class GenericCollectionTypeHandlerTestUnit : GenericCollectionTypeHandlerTestUnitBase
    {
        public void TestSubQuery()
        {
            ...
            var qq = q.Descend(GenericCollectionTestFactory.FieldName);
            var constraint = qq.Constrain(FirstElement(_helper.Elements));
            var set = qq.Execute();
        ...
        {
            var item = RetrieveOnlyInstance(_helper.ItemType);
        ...
        {
            var item = RetrieveOnlyInstance(_helper.ItemType);
            var coll = CollectionFor(item);
            Assert.AreEqual(Count(_helper.Elements), coll.Count);
            var element = FirstElement(coll);
