...
namespace Db4objects.Db4o.Tests.CLI2.Assorted
{
    ...
    {
        private static readonly Action<string> warning =
            delegate(string s)
            {
                TestPlatform.EmitWarning(s + "NullableArraysElementsTestCase: Remove the if() when COR-1130 get closed");
            };

        private readonly TestSubject[] data =
        {
        ...
        {
            var testSubject = QueryByName("foo");
