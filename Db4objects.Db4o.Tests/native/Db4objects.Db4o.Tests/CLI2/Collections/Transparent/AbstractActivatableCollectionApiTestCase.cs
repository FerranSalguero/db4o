...
{
    public abstract partial class AbstractActivatableCollectionApiTestCase<TColl, TElem> : AbstractDb4oTestCase
        where TColl : ICollection<TElem>
    {
        protected static readonly IList<string> Names = new List<string>(new[] {"one", "two", "three", "four"});
        ...
        {
            var item = new CollectionHolder<TColl>(NewPopulatedActivatableCollection());
            Store(item);
        }

        public void TestAdd()
        {
            AssertCollectionChange(delegate(TColl list) { list.Add(NewElement("five")); });
        }
        ...
        public void TestClear()
        {
            ...
            var expected = NewPopulatedPlainCollection();
        ...
        {
            var plainCollection = NewPopulatedPlainCollection();
            var target = new TElem[plainCollection.Count];

            var list = SingleCollection();
        ...
        {
            AssertCollectionChange(delegate(TColl list) { list.Remove(NewElement("one")); });
        }
