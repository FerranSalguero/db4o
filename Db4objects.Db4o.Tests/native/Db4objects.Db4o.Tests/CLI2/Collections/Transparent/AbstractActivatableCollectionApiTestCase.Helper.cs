...
namespace Db4objects.Db4o.Tests.CLI2.Collections.Transparent
{
    abstract partial class AbstractActivatableCollectionApiTestCase<TColl, TElem>
    {
        ...
        {
            var elements = new TElem[NewPopulatedPlainCollection().Count];
            copyAction(elements);

            foreach (var name in Names)
            {
                Assert.IsGreaterOrEqual(0, Array.IndexOf(elements, NewElement(name)));
                Assert.IsGreaterOrEqual(0, Array.IndexOf(elements, NewActivatableElement(name)));
            }
        }
