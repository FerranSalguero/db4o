...
{
    internal class NetArrayFromJavaTestCase : ITestCase, IOptOutSilverlight
    {
#if !CF
        public class Item
        {
        ...
        {
            using (var container = Db4oFactory.OpenFile(DataFile))
            {
...
";
        ...
        {
            return new[] {int.MinValue, 0, int.MaxValue};
        }

        private static float[] FloatArray()
        {
            return new[] {float.MinValue, 0, float.MaxValue, float.NaN};
        }
#endif
    }
}