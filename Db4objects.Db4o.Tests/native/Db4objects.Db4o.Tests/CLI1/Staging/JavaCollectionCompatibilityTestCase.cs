...
namespace Db4objects.Db4o.Tests.CLI1
{
    ...
    internal class JavaCollectionCompatibilityTestCase : TestWithTempFile
    {
        private static readonly string[] ItemNames = {"foo", "bar"};
...
");
        ...
        public void TestArrayList()
        {
            ...
            {
                var holder = RetrieveOnlyInstance<CollectionHolder>(db);
            }
        }

        private void CompileJavaCode()
        {
            var snippet = JavaCode();

            JavaServices.ResetJavaTempPath();
            var @out = JavaServices.CompileJavaCode(snippet.MainClassFile, PopulateItemValues(snippet.SourceCode));
        ...
        protected void Configure(IEmbeddedConfiguration config)
        {
            ...
            config.Common.AddAlias(new TypeAlias("java.util.AbstractCollection",
                TypeName(typeof (DummyAbstractCollection))));
            config.Common.AddAlias(new TypeAlias("java.util.AbstractList", TypeName(typeof (DummyAbstractList))));

            config.Common.AddAlias(new TypeAlias("com.db4o.typehandlers.IgnoreFieldsTypeHandler",
                TypeName(typeof (IgnoreFieldsTypeHandler))));
            config.Common.AddAlias(new TypeAlias("com.db4o.typehandlers.CollectionTypeHandler",
                TypeName(typeof (IgnoreFieldsTypeHandler))));

            config.Common.RegisterTypeHandler(new SingleClassTypeHandlerPredicate(typeof (DummyAbstractList)),
                IgnoreFieldsTypeHandler.Instance);
            config.Common.RegisterTypeHandler(new SingleClassTypeHandlerPredicate(typeof (DummyAbstractCollection)),
                IgnoreFieldsTypeHandler.Instance);
        }
