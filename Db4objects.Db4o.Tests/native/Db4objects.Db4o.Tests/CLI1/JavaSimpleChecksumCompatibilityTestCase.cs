...
namespace Db4objects.Db4o.Tests.CLI1
{
    public class JavaSimpleChecksumCompatibilityTestCase : JavaCompatibilityTestCaseBase
    {
        ...
        public void Test()
        {
            ...
            var bufferLength = Const4.IntLength*ints.Length;
            var buffer = new ByteArrayBuffer(bufferLength);
            for (var i = 0; i < ints.Length; i++)
            {
                buffer.WriteInt(ints[i]);
            }
            var checkSum = CRC32.CheckSum(buffer._buffer, 0, buffer._buffer.Length);
