...
namespace Db4objects.Db4o.Tests.CLI1.NativeQueries
{
    public class ListElementByIdentity : AbstractDb4oTestCase, IOptOutMultiSession
    {
        ...
        public void Test()
        {
            var elem = (LebiElement) Db().QueryByExample(new LebiElement("23"))[0];


            var res = Db().Query(delegate(ListElementByIdentity lebi) { return lebi._list.Contains(elem); });
        ...
        {
            var lebi = new ListElementByIdentity();
