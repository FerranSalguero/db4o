...
//			Expect(new EitherParentByName("Occam"), "Vahin\u00E9", "Achat", "Acrobat");
            ...
            Expect(delegate(Cat cat) { return cat._age == 7; });
            Expect(delegate(Cat cat) { return cat._age == 1; }, "Occam", "Vahin\u00E9");
            Expect(delegate(Cat cat) { return cat._father._age == 1; }, "Achat", "Acrobat", "Vahin\u00E9");
            Expect(delegate(Cat cat) { return cat._father._father._firstName == "Edwin"; }, "Achat", "Acrobat",
                "Vahin\u00E9");
            Expect(delegate(Cat cat)
            {
            ...
            Expect(delegate(Cat cat) { return cat._age + 1 == 2; }, "Occam", "Vahin\u00E9");
            Expect(delegate(Cat cat)
            {
                return cat.GetFirstName() == "Occam"
                       && cat.GetAge() == 1;
            }, "Occam");
            Expect(delegate(Cat cat) { return cat.GetFullName() == "Achat Leo Lenis"; }, "Achat");
            Expect(delegate(Cat cat) { return cat.GetFullName() == null; });
            Expect(delegate(Cat cat) { return cat._firstName.StartsWith("A"); }, "Achat", "Acrobat");
#endif
        }
