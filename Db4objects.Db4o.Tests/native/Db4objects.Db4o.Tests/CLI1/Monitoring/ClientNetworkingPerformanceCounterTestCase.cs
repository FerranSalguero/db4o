...
namespace Db4objects.Db4o.Tests.CLI1.Monitoring
{
    public class ClientNetworkingPerformanceCounterTestCase : PerformanceCounterTestCaseBase, IOptOutMultiSession
    {
        ...
        {
            var fileConfig = Db4oLegacyConfigurationBridge.AsFileConfiguration((Config4Impl) config);
        ...
        public void TestBytesSent()
        {
            ...
            Func<CountingSocket4, double> expectatedValueRetriever =
                delegate(CountingSocket4 s) { return s.BytesSent(); };
            Func<IObjectContainer, double> actualValueRetriever =
                delegate(IObjectContainer container)
                {
                    return PerformanceCounterSpec.NetBytesSentPerSec.PerformanceCounter(container).RawValue;
                };
        ...
        public void TestBytesReceived()
        {
            ...
            Func<CountingSocket4, double> expectatedValueRetriever =
                delegate(CountingSocket4 s) { return s.BytesReceived(); };
            Func<IObjectContainer, double> actualValueRetriever =
                delegate(IObjectContainer container)
                {
                    return PerformanceCounterSpec.NetBytesReceivedPerSec.PerformanceCounter(container).RawValue;
                };

            AssertCounter(operation, expectatedValueRetriever, actualValueRetriever);
        }

        private void AssertCounter(Action<ISocket4> operation, Func<CountingSocket4, double> expectedValueRetriever,
            Func<IObjectContainer, double> actualValueRetriever)
        {
            var container = (ObjectContainerBase) Db();
            container.WithEnvironment(delegate
            {
                var countingSocket = new CountingSocket4(new NullSocket());
