...
{
    internal class PerformanceCounterLifetimeTestCase : TestWithTempFile
    {
        public void TestIOCounters()
        {
            AssertPerformanceCounterInstanceLifetime(new IOMonitoringSupport(),
                delegate(IObjectContainer db) { db.Store(new Item("foo")); });
        }
        ...
        public void TestUnoptimizedNativeQueryCounters()
        {
            AssertPerformanceCounterInstanceLifetime(new NativeQueryMonitoringSupport(), delegate(IObjectContainer db)
            {
                ...
                foreach (var item in db.Query(delegate(Item candidate) { return candidate.Name == "foo"; }))
                {
                }
            });
        }
        ...
        {
            AssertPerformanceCounterInstanceLifetime(new ObjectLifecycleMonitoringSupport(),
                delegate(IObjectContainer db)
                {
        ...
        {
            AssertPerformanceCounterInstanceLifetime(new ReferenceSystemMonitoringSupport(),
                delegate(IObjectContainer db) { db.Store(new Item("foo")); });
        }

        public void TestFreespaceCounters()
        {
            AssertPerformanceCounterInstanceLifetime(new FreespaceMonitoringSupport(),
                delegate(IObjectContainer db) { db.Store(new Item("foo")); });
        }
        ...
        public void TestNetworkingCounters()
        {
            ...
            using (var server = Db4oClientServer.OpenServer(config, TempFile(), Db4oClientServer.ArbitraryPort))
            {
                const string userName = "db4o";
                const string password = userName;

                server.GrantAccess(userName, password);
                var client = Db4oClientServer.OpenClient("localhost", server.Ext().Port(), userName, password);
                client.Close();
        ...
        private void AssertPerformanceCounterInstanceLifetime(IConfigurationItem support,
