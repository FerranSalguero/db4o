...
{
    internal class JavaServerCrossplatformTestCase : CrossplatformTestCaseBase
    {
#if !CF && !SILVERLIGHT
        ...
        public void TestCrossplatform()
        {
            ...
            ReconnectAndRun(
                delegate { AssertInsertFromJavaClient("Michael Sullivan", 2002, DateTime.Now); });
        }
        ...
        public void TestJaggedArray()
        {
            ...
            ReconnectAndRun(
                delegate
                {
                    var result = RunJavaQuery("query-movie", string.Empty);
        ...
        public void _TestQueryByIdentity()
        {
            ...
            ReconnectAndRun(
                delegate
                {
                    ...
                    _client.Commit();
                    return;
                    var result = _client.QueryByExample(new ByExample("foo", foo.Child));
                    Assert.AreEqual(1, result.Count);
                    var actual = (ByExample) result[0];
        ...
        {
            ReconnectAndRun(
                delegate { Iterator4Assert.SameContent(expected, _client.QueryByExample(template).GetEnumerator()); });
        }
        ...
        private void AssertSodaQuery(Person[] expected, string name)
        {
            ReconnectAndRun(
                delegate
                {
                    var actual = SodaQueryByName(name);
        ...
        {
            var eventRegistry = EventRegistryFactory.ForObjectContainer(_client);
        ...
        {
            var expectedOrder = (Person[]) Persons.Clone();
        ...
        private void AssertStoredCount(string name, int count)
        {
            ReconnectAndRun(
                delegate
                {
                    var personList = SodaQueryByName(name);
        ...
        {
            foreach (var person in toBeAdded)
            {
        ...
        protected override void StartServer()
        {
            ThreadPool.QueueUserWorkItem(delegate
            {
                ...
                var output = JavaServices.java(JavaServerCode.MainClassName, Port.ToString(), USER_NAME, USER_PWD,
