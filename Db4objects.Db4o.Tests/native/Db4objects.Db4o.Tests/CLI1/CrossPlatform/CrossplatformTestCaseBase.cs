...
public class ClientCrossPlatform {
	...
	public static class Movies {
        ...
        {
            var javaClient = JavaClientQuery();
            CompileJava(javaClient);

            var queryResult = JavaServices.java(javaClient.MainClassName, Port.ToString(), USER_NAME, USER_PWD, type,
                tbf);
        ...
        protected void ShutdownServer()
        {
            if (_client != null)
            {
                var messageSender = _client.Ext().Configure().ClientServer().GetMessageSender();
        ...
        protected void InsertFromJavaClient(int year, string name, DateTime localReleaseDate)
        {
            ...
            var insertResult = JavaServices.java(javaClient.MainClassName, Port.ToString(), USER_NAME, USER_PWD,
                year.ToString(), name, localReleaseDate.ToShortDateString());
        ...
        private void AssertExactlyOneInstance(string name, int year, DateTime localReleaseDate)
        {
            Iterator4Assert.AreEqual(
                new[] {new Person(name, year, localReleaseDate)},
        ...
        private readonly IDictionary<string, bool> _compiled = new Dictionary<string, bool>();
