...
namespace Db4objects.Db4o.Tests.CLI1.Inside.Query
{
    public class QueryExpressionBuilderTestCase : ITestCase
    {
        ...
        public class ActivatableItem : Item, IActivatable
        {
            ...
            public void ActivateParentForRead()
            {
                parent.Activate(ActivationPurpose.Read);
            }
        ...
        private bool MatchWithActivate(Item item)
        {
        ...
        private bool MatchNoSideEffectsIndirectedActivateOnTypeHierarchy(DerivedActivatableItem item)
        {
        ...
        {
            var expression = ExpressionFromMethod(methodName);
            AssertSimpleComparisionAgainstConstant(expression, "foo");
        }
        ...
        {
            var expression = ExpressionFromPredicate(typeof (NameEqualsTo));
            Assert.AreEqual(
                new ComparisonExpression(
                    NewFieldValue(CandidateFieldRoot.Instance, "name", typeof (string)),
        ...
        {
            return
                new QueryExpressionBuilder().FromMethod(GetType()
