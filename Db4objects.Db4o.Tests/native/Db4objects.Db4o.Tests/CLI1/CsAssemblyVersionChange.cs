...
namespace Db4objects.Db4o.Tests.CLI1
{
    ...
    {
        private const string TestAssemblyName = "test.dll";
...
public class Tester
{
	...
	static void AssertEquals(object expected, object actual)
	{
        ...
        private void ExecuteTestMethodInAppDomain(string basePath, string testMethod)
        {
            ...
            var domain = AppDomain.CreateDomain("db4o-assembly-test-domain", null, setup);
            try
            {
                domain.DoCallBack(new TestMethodRunner(testMethod).Execute);
            }
            finally
            {
        ...
        [Serializable]
        private class TestMethodRunner
        {
            ...
            {
                var testAssembly =
                    Assembly.LoadFrom(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, TestAssemblyName));
                var type = testAssembly.GetType("Tester", true);
