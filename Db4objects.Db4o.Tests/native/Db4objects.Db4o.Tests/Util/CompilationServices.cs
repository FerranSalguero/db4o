...
namespace Db4objects.Db4o.Tests.Util
{
    ...
    public class CompilationServices
    {
        ...
        public static void EmitAssembly(string assemblyFileName, string[] references, params string[] code)
        {
            ...
            var sourceFiles = WriteSourceFiles(Path.GetTempPath(), code);
        ...
        private static string[] WriteSourceFiles(string basePath, string[] code)
        {
            var sourceFiles = new string[code.Length];
            for (var i = 0; i < code.Length; ++i)
            {
                var sourceFile = Path.Combine(basePath, "source" + i + ".cs");
        ...
        private static void WriteFile(string fname, string contents)
        {
        ...
        public static void CompileFiles(string assemblyFName, string[] references, string[] files)
        {
            using (var provider = GetCSharpCodeDomProvider())
            {
                ...
                var results = compiler.CompileAssemblyFromFileBatch(parameters, files);
