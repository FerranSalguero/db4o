...
namespace Db4objects.Db4o.Tests.Util
{
    internal class Certificates
    {
        ...
        {
            var pfxData = CreateSelfSignCertificate(distinguishedName, startTime, endTime, (SecureString) null);
            return pfxData;
        }
            ...
            SecureString password)
        {
            ...
            try
            {
                providerContext = AcquireProviderContext(containerName, PROV_RSA_FULL,
                    NativeMethods.CRYPT_NEWKEYSET + NativeMethods.CRYPT_MACHINE_KEYSET);
                cryptKey = GenerateKey(providerContext, AT_KEYEXCHANGE, NativeMethods.CRYPT_EXPORTABLE);

                var kpi = NewProviderInformationFor(containerName, PROV_RSA_FULL, AT_KEYEXCHANGE);

                certContext = CreateSelfSignedCertificate(providerContext, distinguishedName, startTime, endTime,
                    ref kpi);

                certStore = NativeMethods.CertOpenStore(sz_CERT_STORE_PROV_MEMORY, 0, IntPtr.Zero,
                    CERT_STORE_CREATE_NEW_FLAG, IntPtr.Zero);
                Check(certStore != IntPtr.Zero);

                Check(NativeMethods.CertAddCertificateContextToStore(certStore, certContext, CERT_STORE_ADD_NEW,
            ...
            finally
            {
                ...
                if (providerContext != IntPtr.Zero)
                {
                    ...
                    NativeMethods.CryptAcquireContextW(out providerContext, containerName, null, PROV_RSA_FULL,
        ...
        private static IntPtr CreateSelfSignedCertificate(IntPtr providerContext, string distinguishedName,
            DateTime startTime, DateTime endTime, ref CryptKeyProviderInformation kpi)
        {
            var startSystemTime = ToSystemTime(startTime);
            var endSystemTime = ToSystemTime(endTime);

            var nameData = GetNameData(distinguishedName);
            var dataHandle = GCHandle.Alloc(nameData, GCHandleType.Pinned);
            try
            {
                var nameBlob = new CryptoApiBlob(nameData.Length, dataHandle.AddrOfPinnedObject());
                var certContext = NativeMethods.CertCreateSelfSignCertificate(
                    providerContext,
                    ref nameBlob,
                    0,
                    ref kpi,
        ...
        private static CryptKeyProviderInformation NewProviderInformationFor(string containerName, int providerType,
            int keyexchange)
        {
