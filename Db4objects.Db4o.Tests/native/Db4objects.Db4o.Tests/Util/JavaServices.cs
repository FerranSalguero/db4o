...
namespace Db4objects.Db4o.Tests.Util
{
    internal class JavaServices
    {
        ...
        {
            var db4oVersion = string.Format("{0}.{1}.{2}.{3}", Db4oVersion.Major, Db4oVersion.Minor,
                Db4oVersion.Iteration, Db4oVersion.Revision);
            var distDir = WorkspaceServices.ReadProperty(WorkspaceServices.MachinePropertiesPath(), "dir.dist", true);
            if (string.IsNullOrEmpty(distDir))
            {
                distDir = "db4obuild/dist";
            }
            return
                WorkspaceServices.WorkspacePath(distDir + "/java/lib/db4o-" + db4oVersion + extension + "-java1.2.jar");
        }

        public static void ResetJavaTempPath()
        {
            var tempPath = JavaTempPath;
        ...
        public static string javac(string srcFile)
        {
            ...
            return null;
#else
            Assert.IsTrue(File.Exists(Db4oCoreJarPath()),
                string.Format("'{0}' not found. Make sure the jar was built before running this test.",
                    Db4oCoreJarPath()));
            Assert.IsTrue(File.Exists(Db4oJarPath("-optional")),
                string.Format("'{0}' not found. Make sure the jar was built before running this test.",
                    Db4oJarPath("-optional")));
            Assert.IsTrue(File.Exists(Db4oJarPath("-cs")),
                string.Format("'{0}' not found. Make sure the jar was built before running this test.",
                    Db4oJarPath("-cs")));
