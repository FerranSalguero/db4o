...
namespace Db4objects.Db4o.Linq.Tests.Expressions
{
    ...
    {
        private static readonly int _sid = 42;
        ...
        private static void AssertExpression(string expected, Expression expression)
        {
#if NET_4_0
            const string EqualsSignNotPreceededByExclamation = "(?<!\\!)\\=";
            expected =
                Regex.Replace(expected, EqualsSignNotPreceededByExclamation, "==")
                    .Replace("||", "OrElse")
                    .Replace("&&", "AndAlso");
#endif
        ...
        private static Expression CreateExpression<T>(Expression<Func<Parameter, T>> expression)
        {
            return expression;
        }
