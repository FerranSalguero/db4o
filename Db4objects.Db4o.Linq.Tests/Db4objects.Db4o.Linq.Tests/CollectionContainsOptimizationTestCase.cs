    ...
    public class CollectionContainsOptimizationTestCase : AbstractDb4oLinqTestCase
    {
        ...
        private void AssertQuery<T>(Expression<Func<IObjectContainer, IEnumerable<T>>> queryExpression)
        {
            using (var recorder = new QueryStringRecorder(Db()))
            {
                ...
                var expected = ExpectedRepresentationFor(queryExpression);
                Assert.AreEqual(expected, recorder.QueryString);
            }
        }

        private static string ExpectedRepresentationFor<T>(
            Expression<Func<IObjectContainer, IEnumerable<T>>> queryExpression)
        {
            return string.Format("({0}(Items {1} 'foo'))", ExtentTypeFrom(queryExpression),
                ContainmentCondition(queryExpression));
        }
        ...
        private static string ContainmentCondition<T>(Expression<Func<IObjectContainer, IEnumerable<T>>> queryExpression)
        {
            ...
            var whereMethod = FindMethodCall(queryExpression, "Where");
        ...
        {
            var castMethod = FindMethodCall(queryExpression, "Cast");
            Assert.IsNotNull(castMethod);
            return castMethod.Method.GetGenericArguments()[0].Name;
        }

        private static MethodCallExpression FindMethodCall<T>(
            Expression<Func<IObjectContainer, IEnumerable<T>>> queryExpression, string methodName)
        {
            var expression = queryExpression.Body;
