...
namespace Db4objects.Db4o.Linq.Tests
{
    public class QueryTranslationPerformanceTestCase : AbstractDb4oTestCase
    {
        ...
        public void Test()
        {
            for (var i = 0; i < 3; i++)
            {
                var translationTime = TimeToTranslateQuery();

                Assert.IsSmaller(i == 0 ? TIME_LIMIT_WITH_FIRST_TIME_OVERHEAD : TIME_LIMIT_NO_INITIAL_OVERHEAD,
                    translationTime);
            }
        }
        ...
        private long TimeToTranslateQuery()
        {
            ...
            var elapsedTicks = DateTime.Now.Ticks - start;
