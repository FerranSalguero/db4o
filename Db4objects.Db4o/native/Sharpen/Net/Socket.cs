...
namespace Sharpen.Net
{
    public class Socket : SocketWrapper
    {
        ...
        {
            var found = Dns.GetHostEntry(hostName);
            foreach (var address in found.AddressList)
            {
        ...
        protected override void Initialize(NativeSocket socket)
        {
            base.Initialize(socket);

            var stream = new NetworkStream(_delegate);
