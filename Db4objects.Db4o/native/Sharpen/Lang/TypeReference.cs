...
namespace Sharpen.Lang
{
    ...
    public partial class SimpleTypeReference : TypeReference
    {
        ...
        {
            var other = obj as SimpleTypeReference;
        ...
        private Assembly ResolveAssembly()
        {
            ...
            {
                var unversioned = (AssemblyName) _assemblyName.Clone();
...
#else
            var found = Assembly.LoadWithPartialName(unversioned.FullName);
            return found == null
    ...
    public class GenericTypeReference : SimpleTypeReference
    {
        ...
        public override Type Resolve()
        {
            var baseType = base.Resolve();
            return _genericArguments.Length > 0
                ? baseType.MakeGenericType(Resolve(_genericArguments))
                : baseType;
        }

        private static Type[] Resolve(TypeReference[] typeRefs)
        {
            var types = new Type[typeRefs.Length];
