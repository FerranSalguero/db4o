...
namespace Db4objects.Db4o.Internal
{
    public partial class Config4Impl
    {
        ...
        {
            var csAssembly = Assembly.Load(ClientServerAssemblyName());
            return
                (ILegacyClientServerFactory)
                    Activator.CreateInstance(
                        csAssembly.GetType("Db4objects.Db4o.CS.Internal.Config.LegacyClientServerFactoryImpl"));
        }

        private static string ClientServerAssemblyName()
        {
            var db4oAssembly = typeof (IObjectContainer).Assembly;
            var db4oAssemblySimpleName = db4oAssembly.GetName().Name;
