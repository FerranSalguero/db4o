...
namespace Db4objects.Db4o.Internal.Query
{
    ...
    {
        [NonSerialized] private Delegate _content;
        ...
        private Delegate Unmarshal()
        {
            if (null == _target)
            {
                return Delegate.CreateDelegate(_delegateType, null,
                    _type.GetMethod(_method, BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public));
            }

            return Delegate.CreateDelegate(_delegateType, _target,
                _target.GetType()
                    .GetMethod(_method, BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public));
        }
    }
    ...
    internal class EvaluationDelegateWrapper : DelegateEnvelope, IEvaluation
    {
        ...
        public void Evaluate(ICandidate candidate)
        {
            ...
            var _evaluation = GetEvaluationDelegate();
            _evaluation(candidate);
        }

        private EvaluationDelegate GetEvaluationDelegate()
        {
