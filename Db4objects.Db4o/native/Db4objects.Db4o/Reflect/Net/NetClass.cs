...
namespace Db4objects.Db4o.Reflect.Net
{
    ...
    public class NetClass : IConstructorAwareReflectClass
    {
        ...
        public virtual IReflectMethod GetMethod(string methodName, IReflectClass[] paramClasses)
        {
            try
            {
                var parameterTypes = NetReflector.ToNative(paramClasses);
                var method = GetMethod(_type, methodName, parameterTypes);
                if (method == null)
                {
                    return null;
                }
        ...
        {
            var constructors =
                _type.GetConstructors(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
            var reflectors = new IReflectConstructor[constructors.Length];
        ...
        {
            var fields = Runtime.GetDeclaredFields(_type);
            var reflectors = new IReflectField[fields.Length];
        ...
        {
            var found = Runtime.GetDeclaredMethod(type, methodName, parameterTypes);
        ...
        private void CreateConstructor()
        {
            ...
            _constructor = ConstructorSupport.CreateConstructor(this, _type, _netReflector.Configuration(),
