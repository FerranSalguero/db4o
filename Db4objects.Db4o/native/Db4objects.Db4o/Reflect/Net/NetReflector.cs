...
namespace Db4objects.Db4o.Reflect.Net
{
    public class NetReflector : IReflector
    {
        ...
        public virtual IReflectClass ForClass(Type forType)
        {
            ...
            var underlyingType = GetUnderlyingType(forType);
        ...
        public virtual IReflectClass ForName(string className)
        {
            try
            {
                var type = ReflectPlatform.ForName(className);
        ...
        public virtual bool IsCollection(IReflectClass candidate)
        {
            ...
            var netClass = candidate as NetClass;
            if (null == netClass)
            {
                return false;
            }
            return typeof (ICollection).IsAssignableFrom(netClass.GetNetType());
        }
        ...
        public static Type[] ToNative(IReflectClass[] claxx)
        {
            ...
            if (claxx != null)
            {
                ...
                for (var i = 0; i < claxx.Length; i++)
                {
                    if (claxx[i] != null)
                    {
                        var reflectClass = claxx[i];
        ...
        {
            var netClass = reflectClass.GetDelegate() as NetClass;
            if (netClass == null)
            {
                return null;
            }
