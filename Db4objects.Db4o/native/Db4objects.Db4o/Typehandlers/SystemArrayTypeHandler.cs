...
namespace Db4objects.Db4o.Typehandlers
{
    public class SystemArrayTypeHandler : ICascadingTypeHandler, IVariableLengthTypeHandler, IValueTypeHandler
    {
        ...
        {
            var collection = (Array) obj;
            var elementType = DetectElementTypeHandler(Container(context), collection);
        ...
        {
            var collection = ((ICollection) context.TargetObject());
        ...
        {
            var handler = ReadElementTypeHandler((IReadBuffer) context, context);
        ...
        {
            var elementHandlerId = buffer.ReadInt();
            if (elementHandlerId == 0) return OpenTypeHandlerFrom(context);

            var elementHandler = Container(context).TypeHandlerForClassMetadataID(elementHandlerId);
