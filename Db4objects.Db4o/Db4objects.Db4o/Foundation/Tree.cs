...
namespace Db4objects.Db4o.Foundation
{
    ...
    public abstract class Tree : IShallowClone, IDeepClone, IVisitable
    {
        ...
        public Tree Balance()
        {
            var cmp = _subsequent.Nodes() - _preceding.Nodes();
            if (cmp < -2)
            {
                return RotateRight();
            }
        ...
        public static Tree DeepClone(Tree a_tree, object a_param)
        {
            ...
            var newNode = (Tree) a_tree.DeepClone(a_param);
            newNode._size = a_tree._size;
            newNode._preceding = DeepClone(a_tree._preceding, a_param);
            newNode._subsequent = DeepClone(a_tree._subsequent, a_param);
            return newNode;
        }
        ...
        public Tree Find(Tree template)
        {
            ...
            while (true)
            {
                var comparisonResult = current.Compare(template);
        ...
        public static Tree FindGreaterOrEqual(Tree a_in, Tree a_finder)
        {
            ...
            if (cmp > 0)
            {
                var node = FindGreaterOrEqual(a_in._preceding, a_finder);
                if (node != null)
                {
                    return node;
                }
