...
namespace Db4objects.Db4o.Collections
{
    ...
    /// </seealso>
    /// <seealso cref="Db4objects.Db4o.TA.IActivatable">
    ...
    public partial class ArrayList4<E>
    {
        ...
        {
            ActivateForWrite();
            Array.Copy(elements, sourceIndex, elements, targetIndex, length);
        }

        internal bool AddAllImpl(int index, E[] toBeAdded)
        {
            CheckIndex(index, 0, Count);
            var length = toBeAdded.Length;
            if (length == 0)
            {
                return false;
            }
            ...
            Array.Copy(toBeAdded, 0, elements, index, length);
            IncreaseSize(length);
            MarkModified();
            return true;
        }
        ...
        private void RemoveRangeImpl(int fromIndex, int count)
        {
            ...
            Array.Copy(elements, toIndex, elements, fromIndex, size - toIndex);
        ...
        {
            MarkModified();
            var temp = AllocateStorage(minCapacity);
            Array.Copy(elements, 0, temp, 0, Count);
            elements = temp;
        }
