...
{
    /// <exclude></exclude>
    public abstract partial class ObjectContainerBase : IDisposable, ITransientClass
        , IInternal4, IObjectContainerSpec, IInternalObjectContainer
    {
        private readonly IEnvironment _environment;
        private readonly int _maxStackDepth;

        private readonly ReferenceSystemRegistry _referenceSystemRegistry
            = new ReferenceSystemRegistry();

        protected readonly TimeStampIdGenerator _timeStampIdGenerator = new TimeStampIdGenerator
            ();

        private readonly IntIdGenerator _topLevelCallIdGenerator = new IntIdGenerator();
        ...
        private bool _handlingStackLimitPendings;
        ...
        internal int _showInternalClasses;
        ...
        public bool Close()
        {
            lock (_lock)
            {
                ...
                if (DTrace.enabled)
                {
                    DTrace.CloseCalled.Log(ToString());
                }
                Close1();
                return true;
            }
        ...
        public virtual ClassMetadata ClassMetadataForReflectClass(IReflectClass claxx)
        {
            ...
            var classMetadata = _handlers.ClassMetadataForClass(claxx);
        ...
        public virtual ClassMetadata ClassMetadataForID(int id)
        {
            ...
            var classMetadata = _handlers.ClassMetadataForId(id);
            if (classMetadata != null)
            {
                return classMetadata;
            }
        ...
        public void Activate(Transaction trans, object obj)
        {
            lock (_lock)
            {
        ...
        public void Deactivate(Transaction trans, object obj)
        {
        ...
        {
            var classMetadata = ClassMetadataForObject(obj);
            return DefaultActivationDepth(classMetadata);
        }

        public void Activate(Transaction trans, object obj, IActivationDepth
            depth)
        {
            lock (_lock)
            {
        ...
        public void Bind(Transaction trans, object obj, long id)
        {
            lock (_lock)
            {
                ...
                var intID = (int) id;
                var oldObject = GetByID(trans, id);
                if (oldObject == null)
                {
                    throw new ArgumentException("id");
                }
                var @ref = trans.ReferenceForId(intID);
                if (@ref == null)
                {
                ...
                {
                    var newRef = Bind2(trans, @ref, obj);
            ...
            )
        {
            ...
            var newRef = new ObjectReference(ClassMetadataForObject(obj), id);
        ...
        {
            trans = CheckTransaction(trans);
            var result = AsTopLevelCall(block, trans);
            if (_stackDepth == 0)
            {
        ...
        public virtual void FatalShutdown(Exception origExc)
        {
            ...
            {
                throw new CompositeDb4oException(new[] {origExc, exc});
            }
        ...
        public void Delete1(Transaction trans, object obj, bool userCall)
        {
            ...
            var @ref = trans.ReferenceForObject(obj);
            if (@ref == null)
            {
                return;
            }
        ...
        {
            return _callbacks.CaresAboutDeleting() || yc.HasEventRegistered(SystemTransaction
                (), EventDispatchers.CanDelete);
        }

        private bool CaresAboutDeleted(ClassMetadata yc)
        {
            return _callbacks.CaresAboutDeleted() || yc.HasEventRegistered(SystemTransaction
        ...
        internal virtual object Descend(Transaction trans, object obj, string[] path)
        {
            lock (_lock)
            {
                ...
                var field = (FieldMetadata) foundField.value;
                if (field == null)
                {
                    return null;
                }
                var child = @ref.IsActive()
                    ? field.Get(trans, obj)
                    : DescendMarshallingContext
        ...
        private UnmarshallingContext DescendMarshallingContext(Transaction trans, ObjectReference
            @ref)
        {
            var context = new UnmarshallingContext(trans, @ref, Const4.AddToIdTree
                , false);
        ...
        {
            FatalException(t, Messages.FatalMsgId);
        }
        ...
        internal void FatalException(Exception t, int msgID)
        {
            ...
            Messages.LogErr(ConfigImpl, (msgID == Messages
                .FatalMsgId
                ? 18
                : msgID), null, t);
            if (!IsClosed())
        ...
        public IObjectSet QueryByExample(Transaction trans, object template)
        {
            ...
            {
                trans = CheckTransaction(trans);
                var res = ((IQueryResult) AsTopLevelCall(new _IFunction4_810(this, template
                    ), trans));
        ...
        {
            var obj = ta.ObjectForIdFromCache(id);
            if (obj != null)
            {
        ...
        {
            var obj = AsTopLevelCall(new _IFunction4_892(id), trans);
        ...
        public object GetByUUID(Transaction trans, Db4oUUID uuid)
        {
            lock (_lock)
            {
                ...
                var hardRef = GetHardReferenceBySignature(CheckTransaction(trans)
        ...
        public int GetID(Transaction trans, object obj)
        {
            lock (_lock)
            {
                ...
                var yo = trans.ReferenceForObject(obj);
                if (yo != null)
                {
                    return yo.GetID();
                }
        ...
        public HardObjectReference GetHardObjectReferenceById(Transaction trans, int id)
        {
            ...
            var @ref = trans.ReferenceForId(id);
            if (@ref != null)
            {
                // Take care about handling the returned candidate reference.
            ...
            var readObject = @ref.Read(trans, new LegacyActivationDepth(0), Const4.AddToIdTree
                , true);
            if (readObject == null)
            {
                return HardObjectReference.Invalid;
        ...
        {
            var referenceSystem = _referenceSystemFactory.NewReferenceSystem(this
                );
        ...
        {
            for (var i = 0; i < Const4.EssentialClasses.Length; i++)
            {
        ...
        public virtual ITypeHandler4 TypeHandlerForClass(IReflectClass claxx)
        {
            ...
            var typeHandler = _handlers.TypeHandlerForClass(claxx);
            if (typeHandler != null)
            {
                return typeHandler;
            }
        ...
        public virtual ITypeHandler4 TypeHandlerForClassMetadataID(int id)
        {
            ...
            var classMetadata = ClassMetadataForID(id);
            if (classMetadata == null)
            {
                return null;
            }
            ...
            resetJustPeeked)
        {
            ...
            {
                var ti = new TreeInt(id);
                var tio = (TreeIntObject) Tree.Find(_justPeeked, ti);
                if (tio != null)
                {
                    return tio._object;
                }
            }
            var @ref = PeekReference(trans, id, depth, resetJustPeeked);
        ...
        {
            var reader = RawBufferByAddress(address, length);
        ...
        public virtual ByteArrayBuffer RawBufferByAddress(int address, int length)
        {
            ...
            var reader = new ByteArrayBuffer(length);
        ...
        {
            CheckAddress(address);
            var reader = CreateStatefulBuffer(a_trans, address, length);
        ...
        {
            var renamed = false;
            var i = config.Rename().GetEnumerator();
            while (i.MoveNext())
            {
                var ren = (Rename) i.Current;
        ...
        {
            var classToRename = _classCollection.GetClassMetadata(ren.rFrom);
            if (classToRename == null)
            {
                return false;
            }
            var existing = _classCollection.GetClassMetadata(ren.rTo);
            if (existing != null)
            {
        ...
        {
            var parentClass = _classCollection.GetClassMetadata(ren.rClass);
            if (parentClass == null)
            {
                return false;
            }
        ...
        private void DeleteInverseRenames(Rename rename)
        {
            ...
            var inverseRenames = QueryInverseRenames(rename);
            ...
            , bool checkJust)
        {
            if (obj is IDb4oType)
            {
                var db4oType = Db4oTypeStored(trans, obj);
                if (db4oType != null)
                {
            ...
            )
        {
            ...
            var analyzer = new ObjectAnalyzer(this, obj);
            analyzer.Analyze(trans);
            if (analyzer.NotStorable())
            {
                return 0;
            }
            var @ref = analyzer.ObjectReference();
            if (@ref == null)
            {
                var classMetadata = analyzer.ClassMetadata();
            ...
            depth)
        {
            ...
            var clazz = ReflectorForObject(obj);
            if (clazz.IsArray())
            {
                if (!clazz.GetComponentType().IsPrimitive())
                {
                    var arr = ArrayHandler.Iterator(clazz, obj);
                    while (arr.MoveNext())
                    {
                        ...
                        var classMetadata = ClassMetadataForObject(current);
            ...
            else
            {
                if (depth.Mode().IsDeactivate())
                {
                    ...
                    var metadata = ClassMetadataForObject(obj);
        ...
        private bool ProcessedByImmediateActivation(IActivationContext context)
        {
            ...
            var @ref = context.Transaction().ReferenceForObject(context.TargetObject
                ());
            if (@ref == null)
            {
                return false;
            ...
            updateDepth)
        {
            ...
            _stillToSet = new List4(_stillToSet, new PendingSet(transaction
                , @ref, updateDepth));
        }
        ...
        public IStoredClass StoredClass(Transaction trans, object clazz)
        {
            ...
            {
                trans = CheckTransaction(trans);
                var claxx = ReflectorUtils.ReflectClassFor(Reflector(), clazz);
                if (claxx == null)
                {
                    return null;
                }
                var classMetadata = ClassMetadataForReflectClass(claxx);
                if (classMetadata == null)
                {
                    return null;
                }
        ...
        public virtual IStoredClass[] StoredClasses(Transaction trans)
        {
            ...
            {
                trans = CheckTransaction(trans);
                var classMetadata = _classCollection.StoredClasses();
                var storedClasses = new IStoredClass[classMetadata.Length];
            ...
            )
        {
            ...
            var obj = GetByID2(transaction, id);
            if (obj == null)
            {
                return;
            }
            cascadeDeleteDepth--;
            var claxx = ReflectorForObject(obj);
            if (claxx.IsCollection())
            {
                cascadeDeleteDepth += 1;
            }
            var @ref = transaction.ReferenceForId(id);
            if (@ref == null)
            {
                return;
            }
        ...
        private sealed class _IRunnable_129 : IRunnable
        {
            ...
            public void Run()
            {
                ...
                lock (_enclosing._lock)
                {
                    try
                    {
                        _enclosing._name = _enclosing.ConfigImpl.NameProvider().Name(_enclosing
                            );
                        _enclosing.InitializeReferenceSystemFactory(_enclosing._config);
                        _enclosing.InitializeTransactions();
                        _enclosing.Initialize1(_enclosing._config);
                        _enclosing.OpenImpl();
                        _enclosing.InitializePostOpen();
                        _enclosing.Callbacks().OpenOnFinished(_enclosing);
                        ok = true;
                    }
                    finally
                    {
                        if (!ok)
                        {
                            ...
                            _enclosing.ShutdownObjectContainer();
                        }
                    }
                }
            }
        ...
        private sealed class _IFunction4_189 : IFunction4
        {
            ...
            {
                _enclosing.StillToActivate(_enclosing.ActivationContextFor(((Transaction
                    ) trans), obj, depth));
                _enclosing.ActivatePending(((Transaction) trans));
                return null;
            }
        }
        ...
        private sealed class _IFunction4_399 : IFunction4
        {
            ...
            public object Apply(object trans)
            {
                _enclosing.Commit1(((Transaction) trans));
        ...
        private sealed class _IFunction4_516 : IFunction4
        {
            ...
            public object Apply(object trans)
            {
                _enclosing.DeactivateInternal(((Transaction) trans), obj, _enclosing.ActivationDepthProvider
        ...
        private sealed class _IFunction4_565 : IFunction4
        {
            ...
            public object Apply(object trans)
            {
                _enclosing.Delete2(((Transaction) trans), @ref, obj, 0, userCall);
                return null;
            }
        }
        ...
        private sealed class _IFunction4_810 : IFunction4
        {
            ...
            public object Apply(object trans)
            {
                return _enclosing.QueryByExampleInternal(((Transaction) trans), template);
            }
        }
        ...
        private sealed class _IFunction4_1271 : IFunction4
        {
            ...
            public object Apply(object trans)
            {
                trans = _enclosing.CheckTransaction(((Transaction) trans));
                var @ref = ((Transaction) trans).ReferenceForObject(obj);
                trans = committed ? _enclosing._systemTransaction : ((Transaction) trans);
                object cloned = null;
                if (@ref != null)
                {
                    cloned = _enclosing.PeekPersisted(((Transaction) trans), @ref.GetID(), depth,
                        true);
                }
                return cloned;
            }
        ...
        private sealed class _IFunction4_1599 : IFunction4
        {
            ...
            public object Apply(object trans)
            {
                return _enclosing.StoreAfterReplication(((Transaction) trans), obj, depth, checkJustSet
                    );
            }
        }
