...
namespace Db4objects.Db4o.Internal
{
    ...
    public sealed class DefragmentContextImpl : IReadWriteBuffer, IDefragmentContext
    {
        ...
        private readonly IDefragmentServices _services;
        private readonly ByteArrayBuffer _source;
        private readonly ByteArrayBuffer _target;
        private int _currentParentSourceID;
        private int _declaredAspectCount;

        public DefragmentContextImpl(ByteArrayBuffer source, DefragmentContextImpl
        ...
        {
            var slot = AllocateMappedTargetSlot(sourceAddress, length);
            var sourceBuffer = SourceBufferByAddress(sourceAddress, length);
        ...
        {
            var sourceBuffer = _services.SourceBufferByAddress(sourceAddress, length
                );
            return sourceBuffer;
        }
        ...
        {
            var sourceBuffer = _services.SourceBufferByID(sourceId);
            return sourceBuffer;
        }
        ...
        {
            var typeHandler = HandlerRegistry.CorrectHandlerVersion(this, handler);
        ...
        {
            var value = _source.ReadBitMap(bitCount);
        ...
        private int InternalCopyID(bool flipNegative, int id)
        {
            ...
            var mapped = _services.MappedID(id);
            if (flipped)
            {
                mapped = -mapped;
            }
        ...
        public static void ProcessCopy(IDefragmentServices context, int sourceID, ISlotCopyHandler
            command)
        {
            var sourceReader = context.SourceBufferByID(sourceID);
        ...
        {
            var targetID = services.StrictMappedID(sourceID);
            var targetSlot = services.AllocateTargetSlot(sourceReader.Length());
            services.Mapping().MapId(targetID, targetSlot);
            var context = new DefragmentContextImpl
        ...
        {
            var mapped = _services.MappedID(originalID);
            _target.WriteInt(mapped);
            return mapped;
        }
        ...
        {
            var sourceEntryAddress = _source.ReadInt();
            var sourceId = ConsumeCurrentParentSourceID();
            var sourceObjectAddress = _services.SourceAddressByID(sourceId);
            var entryOffset = sourceEntryAddress - sourceObjectAddress;
            var targetObjectAddress = _services.TargetAddressByID(_services.StrictMappedID(sourceId
                ));
