    ...
    public abstract class PersistentBase : Identifiable, IPersistent, ILinkLengthAware
    {
        ...
        public virtual void Write(Transaction trans)
        {
            ...
            try
            {
                ...
                var slot = container.AllocateSlot(length);
                if (IsNew())
                {
                ...
                var writer = ProduceWriteBuffer(trans, length);
        ...
        public virtual SlotChangeFactory SlotChangeFactory
            ()
        {
            return Slots.SlotChangeFactory.SystemObjects;
        }
    }
}