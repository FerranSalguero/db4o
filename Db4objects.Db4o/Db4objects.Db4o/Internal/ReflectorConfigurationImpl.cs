...
namespace Db4objects.Db4o.Internal
{
    public class ReflectorConfigurationImpl : IReflectorConfiguration
    {
        ...
        {
            var specialized = CallConstructorSpecialized(clazz);
        ...
        {
            var clazzConfig = _config.ConfigClass(clazz.GetName());
            if (clazzConfig != null)
            {
                var res = clazzConfig.CallConstructor();
            ...
            var ancestor = clazz.GetSuperclass();
            if (ancestor != null)
            {
