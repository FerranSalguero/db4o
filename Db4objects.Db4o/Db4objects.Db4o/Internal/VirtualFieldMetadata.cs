...
namespace Db4objects.Db4o.Internal
{
    ...
    public abstract class VirtualFieldMetadata : FieldMetadata
    {
        ...
        private readonly IBuiltinTypeHandler _handler;
            ...
            , bool isNew)
        {
            ...
            var handlers = stream._handlers;
            var migrating = false;
            if (stream._replicationCallState == Const4.New)
            {
                var provider = handlers._replicationReferenceProvider;
                var parentObject = @ref.GetObject();
                var replicationReference = provider.ReferenceFor(parentObject
                    );
                if (replicationReference != null)
                {
                    migrating = true;
                    var va = @ref.ProduceVirtualAttributes();
