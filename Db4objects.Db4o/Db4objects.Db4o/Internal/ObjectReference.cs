...
namespace Db4objects.Db4o.Internal
{
    ...
    public class ObjectReference : Identifiable, IObjectInfo, IActivator
    {
        ...
        public ObjectReference(ClassMetadata classMetadata, int
            id) : this(id)
        {
        ...
        {
            var va = VirtualAttributes(Transaction());
        ...
        public virtual void ActivateOn(Transaction transaction,
        ...
        internal void PeekPersisted(Transaction trans, IActivationDepth
            depth)
        {
            SetObject(Read(trans, depth, Const4.Transient, false));
        }

        internal object Read(Transaction trans, IActivationDepth
        ...
        public object Read(Transaction trans, ByteArrayBuffer buffer
            , object obj, IActivationDepth instantiationDepth, int addToIDTree, bool checkIDTree
            )
        {
            var context = new UnmarshallingContext(trans, buffer, this, addToIDTree
                , checkIDTree);
        ...
        public virtual object ReadPrefetch(Transaction trans, ByteArrayBuffer
        ...
        public void ReadThis(Transaction trans, ByteArrayBuffer
            buffer)
        {
        }
        ...
        internal void Store(Transaction trans, ClassMetadata
            classMetadata, object obj)
        {
            _object = obj;
            _class = classMetadata;
            var id = trans.Container().IdForNewUserObject(trans);
            SetID(id);
        ...
        public virtual VirtualAttributes VirtualAttributes()
        {
            return _virtualAttributes;
        }

        public virtual VirtualAttributes VirtualAttributes(Transaction
        ...
        public virtual void WriteThis(Transaction trans, ByteArrayBuffer
            buffer)
        {
        }

        public virtual void WriteUpdate(Transaction transaction,
        ...
        private ObjectReference Hc_add1(ObjectReference
            newRef)
        {
            var cmp = Hc_compare(newRef);
            if (cmp < 0)
            {
        ...
        {
            var cmp = _hcSubsequent._hcSize - _hcPreceding._hcSize;
            if (cmp < -2)
            {
                return Hc_rotateRight();
            }
        ...
        {
            var cmp = toRef._hcHashcode - _hcHashcode;
            if (cmp == 0)
            {
                cmp = toRef._id - _id;
            }
        ...
        public virtual ObjectReference Hc_find(object obj)
        {
        ...
        private ObjectReference Hc_find(int id, object obj)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (_hcPreceding != null)
                    {
                        var inPreceding = _hcPreceding.Hc_find(id, obj
                            );
                        if (inPreceding != null)
                        {
                            return inPreceding;
        ...
        {
            var hcode = Runtime.IdentityHashCode(obj);
            if (hcode < 0)
            {
                hcode = ~hcode;
            }
        ...
        public virtual ObjectReference Hc_remove(ObjectReference
            findRef)
        {
            if (this == findRef)
            {
        ...
        private ObjectReference Id_add1(ObjectReference
            newRef)
        {
            var cmp = newRef._id - _id;
            if (cmp < 0)
            {
        ...
        {
            var cmp = _idSubsequent._idSize - _idPreceding._idSize;
            if (cmp < -2)
            {
                return Id_rotateRight();
            }
        ...
        public virtual ObjectReference Id_find(int id)
        {
            var cmp = id - _id;
            if (cmp > 0)
            {
        ...
        public virtual ObjectReference Id_remove(ObjectReference
            @ref)
        {
            var cmp = @ref._id - _id;
            if (cmp < 0)
        ...
        public override string ToString()
        {
            ...
            {
                var id = GetID();
                var str = "ObjectReference\nID=" + id;
                var obj = GetObject();
                if (obj == null && _class != null)
                {
                    var container = _class.Container();
                ...
                else
                {
                    ...
                    {
                        var claxx = ClassMetadata().Reflector().ForObject(obj);
                        str += "\n" + claxx.GetName();
                    }
                    str += "\n" + objToString;
                }
