...
namespace Db4objects.Db4o.Internal.Encoding
{
    ...
    public abstract class BuiltInStringEncoding : IStringEncoding
    {
        ...
        /// </remarks>
        private static readonly BuiltInStringEncoding[] AllEncodings =
        {
            ...
            if (encodingByte == 0)
            {
                if (encoding is BuiltInStringEncoding)
                {
                    Runtime.Out.WriteLine(
                        "Warning! Database was created with a custom string encoding but no custom string encoding is configured for this session."
                        );
                }
                return new DelegatingStringIO(encoding);
            }
            var builtInEncoding = AllEncodings[encodingByte];
