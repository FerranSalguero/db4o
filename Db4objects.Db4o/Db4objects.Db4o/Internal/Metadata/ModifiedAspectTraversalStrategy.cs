...
namespace Db4objects.Db4o.Internal.Metadata
{
    ...
    public class ModifiedAspectTraversalStrategy : IAspectTraversalStrategy
    {
        ...
        {
            var currentSlot = 0;
            for (var diffIter = _classDiffs.GetEnumerator(); diffIter.MoveNext();)
            {
                var diff = ((HierarchyAnalyzer.Diff) diffIter.Current);
                var classMetadata = diff.ClassMetadata();
                if (diff.IsRemoved())
                {
        ...
        private sealed class _ITraverseAspectCommandProcessor_49 : ITraverseAspectCommandProcessor
        {
        ...
        private sealed class _ITraverseAspectCommandProcessor_70 : ITraverseAspectCommandProcessor
        {
            public void Process(ITraverseAspectCommand command, ClassAspect currentAspect, int
                currentSlot)
            {
