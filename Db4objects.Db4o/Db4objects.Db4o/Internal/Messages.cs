...
namespace Db4objects.Db4o.Internal
{
    ...
    public sealed class Messages
    {
        ...
        public static string Get(int a_code, string param)
        {
            ...
            var msg = i_messages[a_code];
            if (param != null)
            {
                var pos = msg.IndexOf("%", 0);
                if (pos > -1)
                {
                    msg = Runtime.Substring(msg, 0, pos) + "'" + param + "'" + Runtime.Substring
                        (msg, pos + 1);
                }
            }
            return msg;
        ...
        private static void Load()
        {
            if (i_messages == null)
            {
                i_messages = new[]
                {
                    string.Empty, "blocksize should be between 1 and 127"
                    , "% close request", "% closed", "Exception opening %", "% opened O.K.",
                    ...
                    , "File % not available for readwrite access.", "File read access failed.",
                    "File not found: % Creating new file"
                    , "Creation of file failed: %", "File write failed.", "File format incompatible: %"
                    , "Uncaught Exception. Engine closed.", "writing log for %",
                    ...
                    , "Please mail the following to exception@db4o.com:\n <db4o " + Db4oVersion.Name
                      + " stacktrace>",
                    "</db4o " + Db4oVersion.Name + " stacktrace>", "Creation of lock file failed: %"
                    ...
                    , "Client % timed out and closed.", "Connection closed by client %.",
                    ...
                    "Add constructor that won't throw exceptions, configure constructor calls, configure exceptionsOnNotStorable(false) or provide a translator to class % and make sure the class is deployed to the server with the same package/namespace + assembly name."
                    ...
                    , "Password can not be null.", "Classes does not match.",
                    ...
                    "This functionality is not supported for db4o clients in Client/Server mode."
        ...
        public static void LogErr(IConfiguration config, int code, string msg, Exception
            t)
        {
            var ps = ((Config4Impl) SafeConfig(config)).ErrStream();
            new MessageOutput(msg, code, ps);
            if (t != null)
            {
                new MessageOutput(null, 25, ps);
                Runtime.PrintStackTrace(t, ps);
                new MessageOutput(null, 26, ps, false);
            }
        }

        [Obsolete(@"uses deprecated api")]
