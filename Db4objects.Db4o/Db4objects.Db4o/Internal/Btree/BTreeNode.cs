...
namespace Db4objects.Db4o.Internal.Btree
{
    ...
    public sealed class BTreeNode : LocalPersistentBase
    {
        ...
        /// </returns>
        public BTreeNode Add(Transaction trans, IPreparedComparison
            preparedComparison, object obj)
        {
            var reader = PrepareRead(trans);
            var s = Search(trans, preparedComparison, reader);
            if (_isLeaf)
            {
                PrepareWrite(trans);
                SetStateDirty();
            ...
            {
                var childNode = Child(reader, s.Cursor());
                var childNodeOrSplit = childNode.Add(trans,
        ...
        {
            var patch = (BTreeUpdate) KeyPatch(index);
            var nextPatch = patch.RemoveFor(trans);
        ...
        private bool WasRemoved(Transaction trans, Searcher s)
        {
            ...
            var patch = KeyPatch(trans, s.Cursor());
        ...
        {
            var reader = PrepareRead(trans);
            var s = Search(trans, preparedComparison, reader, target);
            if (!_isLeaf)
            {
            ...
            {
                var res = FindLowestLeafMatch(trans, preparedComparison, s.Cursor
                    () - 1);
                if (res != null)
                {
                    return res;
        ...
        internal BTreeNode Child(int index)
        {
            if (_children[index] is BTreeNode)
            {
        ...
        internal BTreeNode Child(ByteArrayBuffer reader, int
            index)
        {
        ...
        private bool ChildLoaded(int index)
        {
            ...
            return _children[index] is BTreeNode;
        }
        ...
        internal void CommitOrRollback(Transaction trans, bool isCommit)
        {
            ...
            var keyZero = _keys[0];
            var tempKeys = new object[_btree.NodeSize()];
            var count = 0;
            for (var i = 0; i < _count; i++)
            {
                var key = _keys[i];
                var patch = KeyPatch(i);
                if (patch != null)
                {
                    key = isCommit
                        ? patch.Commit(trans, _btree, this)
                        : patch.Rollback(trans, _btree
                            );
                }
                if (key != No4.Instance)
                {
        ...
        internal void HoldChildrenAsIDs()
        {
            ...
            for (var i = 0; i < _count; i++)
            {
                if (_children[i] is BTreeNode)
                {
                    _children[i] = ((BTreeNode) _children[i]).GetID();
                }
            }
        }
        ...
        private void PrepareInsert(int pos)
        {
            ...
            var len = _count - pos;
            Array.Copy(_keys, pos, _keys, pos + 1, len);
            if (_children != null)
            {
                Array.Copy(_children, pos, _children, pos + 1, len);
            }
            _count++;
        }
        ...
        private void Remove(int pos)
        {
            ...
            var len = _count - pos;
            _count--;
            Array.Copy(_keys, pos + 1, _keys, pos, len);
            _keys[_count] = null;
            if (_children != null)
            {
                Array.Copy(_children, pos + 1, _children, pos, len);
        ...
        public void Remove(Transaction trans, int index)
        {
            ...
            {
                var transPatch = patch.ForTransaction(trans);
                if (transPatch != null)
                {
        ...
        public bool Remove(Transaction trans, object obj, int index)
        {
            ...
            if (transPatch != null)
            {
                ...
                {
                    var removePatch = ApplyNewRemovePatch(trans, transPatch.GetObject());
        ...
        {
            var s = new Searcher(target, _count);
            if (CanWrite())
            {
                while (s.Incomplete())
                {
            ...
            )
        {
            for (var i = 0; i < _count; i++)
            {
                var childFirstPointer = Child(reader, i).FirstPointer(trans);
        ...
        private BTreePointer BranchLastPointer(Transaction trans, ByteArrayBuffer reader)
        {
            ...
            {
                var childLastPointer = Child(reader, i).LastPointer(trans);
        ...
        internal void TraverseAllNodes(Transaction trans, IVisitor4 command)
        {
            ...
            for (var childIdx = 0; childIdx < _count; childIdx++)
            {
