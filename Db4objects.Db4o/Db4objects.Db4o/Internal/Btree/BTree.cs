...
namespace Db4objects.Db4o.Internal.Btree
{
    ...
    {
        private const byte BtreeVersion = 1;
        ...
        {
            KeyCantBeNull(key);
            var preparedComparison = _keyHandler.PrepareComparison(trans.Context
                (), key);
        ...
        {
            var bTreePointer = SearchPointer(trans, key);
            if (bTreePointer == null)
            {
                return null;
            }
            var result = bTreePointer.Key();
            Enlist(trans);
            var preparedComparison = KeyHandler().PrepareComparison(trans.Context
                (), key);
            var node = bTreePointer.Node();
        ...
        public virtual BTreePointer SearchPointer(Transaction trans, object key)
        {
            ...
            var preparedComparison = KeyHandler().PrepareComparison(trans.Context
                (), key);
            var start = SearchLeaf(trans, preparedComparison, SearchTarget.
                Lowest);
            var bTreePointer = start.FirstValidPointer();
            if (bTreePointer == null)
            {
                ConvertCacheEvictedNodesToReadMode();
                return null;
            }
            var found = bTreePointer.Key();
            ConvertCacheEvictedNodesToReadMode();
            ...
            )
        {
            ...
            var start = SearchLeaf(trans, preparedComparison, SearchTarget.
                Lowest);
            var end = SearchLeaf(trans, preparedComparison, SearchTarget.Highest
                );
            var range = start.CreateIncludingRange(end);
            ConvertCacheEvictedNodesToReadMode();
            return range;
        }
        ...
        {
            EnsureActive(trans);
            var result = _root.SearchLeaf(trans, preparedComparison, target
                );
            ConvertCacheEvictedNodesToReadMode();
            return result;
        }
        ...
        {
            var sizeInTransaction = SizeIn(transaction);
            var sizeModification = (((int) sizeInTransaction.value));
        ...
        public virtual BTreePointer FirstPointer(Transaction trans)
        {
            ...
            var pointer = _root.FirstPointer(trans);
            ConvertCacheEvictedNodesToReadMode();
            return pointer;
        }
        ...
        public virtual BTreePointer LastPointer(Transaction trans)
        {
            ...
            var pointer = _root.LastPointer(trans);
            ConvertCacheEvictedNodesToReadMode();
            return pointer;
        }

        public virtual BTree DebugLoadFully(Transaction trans
            )
        {
            EnsureActive(trans);
        ...
        {
            var preparedComparison = _keyHandler.PrepareComparison(context, key1
                );
        ...
        {
            var allNodeIDs = new Collection4();
        ...
        {
            var currentPointer = FirstPointer(transaction);
            while (currentPointer != null && currentPointer.IsValid())
            {
                var node = currentPointer.Node();
                var index = currentPointer.Index();
        ...
        private sealed class _IVisitor4_294 : IVisitor4
        {
            public void Visit(object obj)
            {
                var node = (BTreeNode) ((TreeIntObject) obj).GetObject();
                node.Purge();
            }
        }
        ...
        private sealed class _IVisitor4_311 : IVisitor4
        {
            ...
            {
                _enclosing._processing.Add(((TreeIntObject) node).GetObject());
            }
        }
        ...
        private sealed class _ISlotCopyHandler_481 : ISlotCopyHandler
        {
            ...
            {
                _enclosing.DefragIndex(context);
            }
        }
