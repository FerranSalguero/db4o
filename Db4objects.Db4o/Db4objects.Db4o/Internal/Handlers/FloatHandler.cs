...
namespace Db4objects.Db4o.Internal.Handlers
{
    public class FloatHandler : IntHandler
    {
        ...
        {
            WriteInt(Runtime.FloatToIntBits(((float) a_object)), a_bytes);
        }

        public override object Read(IReadContext context)
        {
            return Runtime.IntBitsToFloat(context.ReadInt());
        }

        public override void Write(IWriteContext context, object obj)
        {
            context.WriteInt(Runtime.FloatToIntBits(((float) obj)));
        }

        public override IPreparedComparison InternalPrepareComparison(object source)
        {
            var sourceFloat = ((float) source);
