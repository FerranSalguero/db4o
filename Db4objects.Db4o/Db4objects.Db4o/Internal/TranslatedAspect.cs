...
namespace Db4objects.Db4o.Internal
{
    public sealed class TranslatedAspect : FieldMetadata
    {
        ...
        {
            var stream = ContainingClass().Container();
            var storedClass = stream.Reflector().ForClass(TranslatorStoredClass(_translator
                ));
            Configure(storedClass, false);
            var baseType = Handlers4.BaseType(storedClass);
        ...
        {
            return Internal.Marshall.AspectType.Translator;
        }
        ...
        public object Construct(ObjectReferenceContext context)
        {
            ...
            var fieldHasValue = ContainingClass().SeekToField(context, this);
            try
            {
