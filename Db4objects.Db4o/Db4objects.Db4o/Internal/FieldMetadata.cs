...
namespace Db4objects.Db4o.Internal
{
    ...
    {
        internal static readonly FieldMetadata[] EmptyArray = new
            FieldMetadata[0];

        private readonly ClassMetadata _containingClass;
        ...
        {
            var convertedIndexEntry = IndexEntryFor(indexEntry);
        ...
        {
            var indexableTypeHandler = (IIndexableTypeHandler) HandlerRegistry
        ...
        {
            var id = _fieldType != null ? _fieldType.GetID() : 0;
            if (_fieldTypeID == 0)
            {
                _fieldTypeID = id;
                return;
        ...
        private bool CanHold(IReflectClass type)
        {
            ...
            var classReflector = FieldType().ClassReflector();
        ...
        public override sealed void CascadeActivation(IActivationContext context)
        {
            if (!Alive())
            {
                return;
            }
            var cascadeTo = CascadingTarget(context);
            if (cascadeTo == null)
            {
                return;
            }
            var cascadeContext = context.ForObject(cascadeTo);
            var classMetadata = cascadeContext.ClassMetadata();
        ...
        private void EnsureObjectIsActive(IActivationContext context)
        {
            ...
            var classMetadata = container.ClassMetadataForObject(context.TargetObject
                ());
        ...
        {
            var obj = GetOn(trans, a_template);
            if (obj != null)
            {
                var objs = Platform4.FlattenCollection(trans.Container(), obj);
                var j = objs.GetEnumerator();
                while (j.MoveNext())
                {
                    obj = j.Current;
                    if (obj != null)
                    {
                        ...
                        {
                            var nullValue = _reflectField.GetFieldType().NullValue();
                        ...
                        {
                            var constraint = new QConObject(trans, a_parent, QField(trans), obj);
        ...
        /// <exception cref="Db4objects.Db4o.Internal.FieldIndexException"></exception>
        public override sealed void CollectIDs(CollectIdContext context)
        {
            if (!Alive())
            {
            ...
            var handler = HandlerRegistry.CorrectHandlerVersion(context, GetHandler
                ());
        ...
        internal virtual void Configure(IReflectClass clazz, bool isPrimitive)
        {
            ...
            if (_isArray)
            {
                var reflectArray = Reflector().Array();
        ...
        public override void Deactivate(IActivationContext context)
        {
            ...
            var isEnumClass = _containingClass.IsEnum();
            if (_isPrimitive && !_isArray)
            {
                if (!isEnumClass)
                {
        ...
        public override void Delete(DeleteContextImpl context, bool isUpdate)
        {
            ...
            try
            {
                ...
                var childContext = new DeleteContextImpl(context, GetStoredType(),
                    _config);
        ...
        private void RemoveIndexEntry(DeleteContextImpl context)
        {
            ...
            var obj = ReadIndexEntry(context);
        ...
        public object Get(Transaction trans, object onObject)
        {
            ...
            lock (container.Lock())
            {
                ...
                var @ref = trans.ReferenceForObject(onObject);
                if (@ref == null)
                {
                    return null;
                }
        ...
        public virtual object GetOrCreate(Transaction trans, object onObject)
        {
            ...
            var obj = FieldAccessor().Get(_reflectField, onObject);
        ...
        {
            var containingClassConfig = _containingClass.Config();
        ...
        public virtual void AttemptUpdate(UnmarshallingContext context)
        {
            ...
            try
            {
                var toSet = context.Read(GetHandler());
                if (toSet != null)
                {
                    Set(context.PersistentObject(), toSet);
                }
        ...
        {
            var claxx = _containingClass.ClassReflector();
            if (claxx == null)
            {
                return null;
            }
            ...
            var fieldType = _reflectField.GetFieldType();
            if (fieldType == null)
            {
                return null;
            }
        ...
        {
            var currentFieldType = DetectFieldType();
        ...
        {
            // alive needs to be checked by all callers: Done
            var updateDepth = context.UpdateDepth();
        ...
        public virtual QField QField(Transaction
            a_trans)
        {
            var classMetadataID = 0;
            if (_containingClass != null)
            {
                classMetadataID = _containingClass.GetID();
            }
            return new QField(a_trans, _name, this,
        ...
        {
            var newFieldType = DetectFieldType();
        ...
        public void TraverseValues(Transaction transaction, IVisitor4 userVisitor)
        {
            ...
            AssertHasIndex();
            var stream = transaction.Container();
            if (stream.IsClient)
            {
                Exceptions4.ThrowRuntimeException(Messages.ClientServerUnsupported
                    );
            }
            lock (stream.Lock())
            {
        ...
        {
            var stream = systemTrans.Container();
            var indexHandler = IndexHandler(stream);
        ...
        protected virtual IIndexable4 IndexHandler(ObjectContainerBase stream)
        {
            ...
            var indexType = _reflectField.IndexType();
            var classHandler = TypeHandlerForClass(stream, indexType);
        ...
        {
            AssertHasIndex();
            var transActionalValue = Handlers4.WrapWithTransactionContext(transaction, value
                , GetHandler());
            var lowerBound = SearchLowerBound(transaction, transActionalValue
                );
            var upperBound = SearchUpperBound(transaction, transActionalValue
                );
        ...
        {
            var writer = stream.ReadStatefulBufferById(stream.SystemTransaction(),
                objectId);
            if (writer != null)
            {
        ...
        protected virtual void RebuildIndexForWriter(LocalObjectContainer stream, StatefulBuffer
            writer, int objectId)
        {
            var oh = new ObjectHeader(stream, writer);
            var obj = ReadIndexEntryForRebuild(writer, oh);
        ...
        {
            var classMetadata = oh.ClassMetadata();
            if (classMetadata == null)
            {
                return DefaultValueForFieldType();
            }
            var context = new ObjectIdContextImpl(writer.Transaction(), writer
                , oh, writer.GetID());
        ...
        {
            var handler = _fieldType.TypeHandler();
        ...
        public override void DefragAspect(IDefragmentContext context)
        {
            ...
            var correctTypeHandlerVersion = HandlerRegistry.CorrectHandlerVersion(context
        ...
        {
            return Internal.Marshall.AspectType.Field;
        }
        ...
        private sealed class _IClosure4_443 : IClosure4
        {
            ...
            public object Run()
            {
                childContext.Delete(_enclosing.GetHandler());
                return null;
            }
        }
