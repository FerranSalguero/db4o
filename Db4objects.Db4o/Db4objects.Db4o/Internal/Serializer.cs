...
namespace Db4objects.Db4o.Internal
{
    ...
    public class Serializer
    {
        ...
        {
            var serialized = Marshall(ta.Container(), obj);
            var buffer = new StatefulBuffer(ta, serialized.Length());
        ...
        public static SerializedGraph Marshall(ObjectContainerBase serviceProvider, object
            obj)
        {
            var memoryBin = new MemoryBin(223, GrowthStrategy());
            var carrier = NewTransportObjectContainer(serviceProvider, memoryBin
                );
            carrier.ProduceClassMetadata(carrier.Reflector().ForObject(obj));
            carrier.Store(obj);
            var id = (int) carrier.GetID(obj);
            carrier.Close();
        ...
        {
            var container = new TransportObjectContainer(serviceProvider
                , memoryBin);
            ...
            , int id)
        {
            ...
            var memoryBin = new MemoryBin(bytes, GrowthStrategy());
            var carrier = NewTransportObjectContainer(serviceProvider, memoryBin
                );
            var obj = carrier.GetByID(id);
