...
namespace Db4objects.Db4o.Internal
{
    ...
    public class Config4Class : Config4Abstract, IObjectClass, IDeepClone
    {
        ...
        private static readonly KeySpec TranslatorNameKey = new KeySpec(null);
        private static readonly KeySpec UpdateDepthKey = new KeySpec(Const4.Unspecified);
        private static readonly KeySpec WriteAsKey = new KeySpec(null);
        ...
        {
            var parentConfig = ((Config4Impl.ConfigDeepCloneContext) param)._cloned;
        ...
        {
            var exceptionalFieldsCollection = ExceptionalFields();
            var c4f = (Config4Field) exceptionalFieldsCollection.Get(fieldName);
            if (c4f == null)
            {
        ...
        {
            var cascadeOnActivate = CascadeOnActivate();
            ...
            {
                var minimumActivationDepth = MinimumActivationDepth();
                if (minimumActivationDepth != 0 && depth < minimumActivationDepth)
                {
                    depth = minimumActivationDepth;
                }
                var maximumActivationDepth = MaximumActivationDepth();
        ...
        {
            var exceptionalFields = ExceptionalFieldsOrNull();
            if (exceptionalFields == null)
            {
                return null;
            }
            var config4Field = (Config4Field) exceptionalFields.Get(fieldName);
        ...
        {
            var translator = (IObjectTranslator) _config.Get(TranslatorKey);
            if (translator != null)
            {
                return translator;
            }
            var translatorName = _config.GetAsString(TranslatorNameKey);
        ...
        {
            return (IObjectTranslator) Activator.CreateInstance(ReflectPlatform.ForName
                (translatorName));
        }
        ...
        {
            var exceptionalFieldsCollection = ExceptionalFieldsOrNull();
