    ...
    public class IndexedNodeCollector
    {
        ...
        private void ImplicitlyAndJoinsOnSameField()
        {
            ...
            for (var i = 0; i < nodes.Length; i++)
            {
                ...
                {
                    var current = (OrIndexedLeaf) node;
                    var other = FindJoinOnSameFieldAtSameLevel(current);
                    if (null != other)
                    {
        ...
        private OrIndexedLeaf FindJoinOnSameFieldAtSameLevel(OrIndexedLeaf join)
        {
            ...
            while (i.MoveNext())
            {
                ...
                {
                    var current = (OrIndexedLeaf) i.Current;
        ...
        {
            var existing = FindLeafOnSameField(conObject);
            if (existing != null)
            {
        ...
        {
            var joins = CollectTopLevelJoins(constraintWithJoins);
        ...
        {
            var i = leaves.GetEnumerator();
            i.MoveNext();
            var first = (QCon) i.Current;
            while (i.MoveNext())
            {
        ...
        {
            var i = joins.GetEnumerator();
            while (i.MoveNext())
            {
                var join = ((QConJoin) i.Current);
        ...
        {
            var leaves = CollectLeaves(joins);
            ...
            )
        {
            var i = joins.GetEnumerator();
            i.MoveNext();
            var last = NodeForConstraint((QCon) i.Current);
            while (i.MoveNext())
            {
                var node = NodeForConstraint((QCon) i.Current);
        ...
        {
            var i = constraintWithJoins.IterateJoins();
            while (i.MoveNext())
            {
                var join = (QConJoin) i.Current;
                if (!join.HasJoins())
        ...
        {
            var c1 = NodeForConstraint(join.Constraint1());
            var c2 = NodeForConstraint(join.Constraint2());
            if (join.IsOr())
            {
        ...
        {
            var node = (IIndexedNodeWithRange) _nodeCache.Get(con);
        ...
        private IndexedLeaf FindLeafOnSameField(QConObject conObject)
        {
            ...
            while (i.MoveNext())
            {
                ...
                {
                    var leaf = (IndexedLeaf) i.Current;
