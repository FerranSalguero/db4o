...
namespace Db4objects.Db4o.Internal.Fieldindex
{
    public class FieldIndexProcessor
    {
        ...
        public virtual FieldIndexProcessorResult Run()
        {
            ...
            {
                var resolved = ResolveFully(bestIndex);
                if (null == resolved)
                {
                    return FieldIndexProcessorResult.NoIndexFound;
                }
        ...
        {
            var i = CollectIndexedNodes();
            if (!i.MoveNext())
            {
                return null;
            }
            var best = (IIndexedNode) i.Current;
            while (i.MoveNext())
            {
                var leaf = (IIndexedNode) i.Current;
