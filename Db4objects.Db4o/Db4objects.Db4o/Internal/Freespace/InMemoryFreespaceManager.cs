...
namespace Db4objects.Db4o.Internal.Freespace
{
    public class InMemoryFreespaceManager : AbstractFreespaceManager
    {
        ...
        {
            var addressNode = new FreeSlotNode(address);
        ...
        {
            var sizeNode = (FreeSlotNode) Tree.Last(_freeBySize);
        ...
        public override void Free(Slot slot)
        {
            ...
            var addressnode = (FreeSlotNode) Tree.FindSmaller(_freeByAddress, _finder
                );
                ...
                ))
            {
                ...
                var secondAddressNode = (FreeSlotNode) Tree.FindGreaterOrEqual(_freeByAddress
                    , _finder);
        ...
        public override Slot AllocateSlot(int length)
        {
            ...
            var node = (FreeSlotNode) _finder._object;
            _listener.SlotRemoved(node._key);
            var blocksFound = node._key;
            var address = node._peer._key;
            _freeByAddress = _freeByAddress.RemoveNode(node._peer);
            var remainingBlocks = blocksFound - length;
        ...
        private void Read(ByteArrayBuffer reader)
        {
            ...
            var addressTree = ByRef.NewInstance();
            if (_freeBySize != null)
            {
        ...
        private sealed class _IVisitor4_176 : IVisitor4
        {
            ...
            {
                var node = ((FreeSlotNode) a_object)._peer;
