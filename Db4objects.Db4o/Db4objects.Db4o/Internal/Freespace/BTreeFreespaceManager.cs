...
namespace Db4objects.Db4o.Internal.Freespace
{
    ...
    {
        private readonly InMemoryFreespaceManager _delegate;
        ...
        {
            var config = new BTreeConfiguration(_idSystem, SlotChangeFactory.FreeSpace
                , 64, false);
        ...
        public override void Free(Slot slot)
        {
            ...
            try
            {
                ...
                var remove = new Slot[2];
                var newFreeSlot = slot;
                var pointer = SearchBTree(_slotsByAddress, slot, SearchTarget.Lowest);
                var previousPointer = pointer != null
                    ? pointer.Previous()
                    : _slotsByAddress
                        .LastPointer(Transaction());
                if (previousPointer != null)
                {
                    var previousSlot = (Slot) previousPointer.Key();
        ...
        public override Slot AllocateSlot(int length)
        {
            ...
            {
                BeginDelegation();
                var pointer = SearchBTree(_slotsByLength, new Slot(0, length), SearchTarget
                    .Highest);
                if (pointer == null)
                {
                    return null;
                }
                var slot = (Slot) pointer.Key();
                RemoveSlot(slot);
                var remainingLength = slot.Length() - length;
        ...
        {
            var searchResult = bTree.SearchLeaf(Transaction(), slot, target
                );
