...
namespace Db4objects.Db4o.Internal
{
    ...
    public class MarshallingBuffer : IWriteBuffer
    {
        ...
        {
            var newSize = _delegate.Length()*2;
            if (newSize - _lastOffSet < sizeNeeded)
            {
                newSize += sizeNeeded;
            }
            var temp = new ByteArrayBuffer(newSize);
            ...
            )
        {
            ...
            var length = _delegate.Offset() - _lastOffSet;
            other.PrepareWrite(length);
            var otherOffset = other._delegate.Offset();
            Array.Copy(_delegate._buffer, _lastOffSet, other._delegate._buffer, otherOffset
                , length);
        ...
        public virtual void TransferContentTo(ByteArrayBuffer buffer, int length)
        {
            ...
            Array.Copy(_delegate._buffer, 0, buffer._buffer, buffer._offset, length);
        ...
        {
            var child = new MarshallingBuffer();
        ...
        {
            var length = storeLengthInLink ? Const4.IntLength*2 : Const4.IntLength;
            PrepareWrite(length);
            ...
            )
        {
            if (_indexedField != null)
            {
                ...
                var buffer = new StatefulBuffer(context.Transaction(), UnblockedLength
                    ());
                var blockedPosition = context.Container().BlockConverter().BytesToBlocks(position
                    );
                var indexID = masterAddress + blockedPosition;
        ...
        {
            var totalLength = context.Container().BlockConverter().BlockAlignedBytes(precedingLength
                                                                                     + Length());
            var newLength = totalLength - precedingLength;
        ...
        private void BlockAlign(int length)
        {
            ...
            {
                var sizeNeeded = length - _delegate.Offset();
                PrepareWrite(sizeNeeded);
            }
        ...
        private sealed class _IReservedBuffer_178 : IReservedBuffer
        {
            ...
            public _IReservedBuffer_178(MarshallingBuffer _enclosing)
            {
                ...
                reservedOffset = this._enclosing._delegate.Offset();
            }

            public void WriteBytes(byte[] bytes)
            {
                var currentOffset = _enclosing._delegate.Offset();
                _enclosing._delegate.Seek(reservedOffset);
                _enclosing._delegate.WriteBytes(bytes);
                _enclosing._delegate.Seek(currentOffset);
            }
        }
    }
}