...
namespace Db4objects.Db4o.Defragment
{
    ...
    public class Defragment
    {
        ...
        {
            var storage = config.Db4oConfig().Storage;
            EnsureFileExists(storage, config.OrigPath());
            var backupStorage = config.BackupStorage();
            ...
            var services = new DefragmentServicesImpl(config, listener);
            try
            {
        ...
        {
            var origStorage = config.Db4oConfig().Storage;
        ...
        {
            var origBin = sourceStorage.Open(new BinConfiguration(sourcePath, true, 0, true)
                );
            try
            {
                var backupBin = targetStorage.Open(new BinConfiguration(targetPath, true, origBin
                    .Length(), false));
                try
                {
        ...
        private static void UpgradeFile(DefragmentConfig config)
        {
            CopyBin(config.BackupStorage(), config.BackupStorage(), config.BackupPath(), config
                .TempPath());
            var db4oConfig = (IConfiguration) ((Config4Impl) config.Db4oConfig()).DeepClone
                (null);
            db4oConfig.Storage = config.BackupStorage();
            db4oConfig.AllowVersionUpdates(true);
            var db = Db4oFactory.OpenFile(db4oConfig, config.TempPath());
            db.Close();
        }

        private static void DefragUnindexed(DefragmentServicesImpl services)
        {
            var unindexedIDs = services.UnindexedIDs();
            while (unindexedIDs.HasMoreIds())
            {
                var origID = unindexedIDs.NextId();
            ...
            IPassCommand command)
        {
            ...
            var classes = context.StoredClasses(DefragmentServicesImpl.Sourcedb);
            for (var classIdx = 0; classIdx < classes.Length; classIdx++)
            {
                var classMetadata = (ClassMetadata) classes[classIdx];
            ...
            var uuidIndex = context.SourceUuidIndex();
            if (uuidIndex != null)
            {
        ...
        {
            var parentClass = curClass.GetAncestor();
        ...
        private static void ProcessClassAndFieldIndices(DefragmentServicesImpl context, ClassMetadata
            curClass, IPassCommand command)
        {
            var sourceClassIndexID = 0;
            var targetClassIndexID = 0;
            ...
            curClass, IPassCommand command)
        {
            if (curClass.HasClassIndex())
            {
                var indexStrategy = (BTreeClassIndexStrategy) curClass.Index();
                var btree = indexStrategy.Btree();
