...
namespace Db4objects.Db4o.IO
{
    ...
    public class MemoryStorage : IStorage
    {
        ...
        {
            var storage = ProduceStorage(config);
        ...
        {
            var bin = ((MemoryBin) Sharpen.Collections.Remove(_bins, oldUri));
            if (bin == null)
            {
        ...
        private IBin ProduceStorage(BinConfiguration config)
        {
            ...
            var newStorage = new MemoryBin(new byte[(int) config.InitialLength()], _growthStrategy
                );
