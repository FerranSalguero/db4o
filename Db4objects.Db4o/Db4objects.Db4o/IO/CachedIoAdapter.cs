...
namespace Db4objects.Db4o.IO
{
    ...
    {
        private static readonly int DefaultPageSize = 1024;
        private static readonly int DefaultPageCount = 64;
        ...
        public override IoAdapter Open(string path, bool lockFile, long initialLength, bool
            readOnly)
        {
            return new CachedIoAdapter(path, lockFile, initialLength, readOnly
        ...
        {
            var startAddress = _position;
            var bytesToRead = length;
            var totalRead = 0;
            while (bytesToRead > 0)
            {
                var page = GetPage(startAddress, true);
        ...
        {
            var page = GetPageFromCache(startAddress);
            if (page != null)
            {
                if (ContainsHeaderBlock(page))
                {
        ...
        private bool ContainsHeaderBlock(Page page)
        {
        ...
        /// <exception cref="Db4objects.Db4o.Ext.Db4oIOException"></exception>
        private void GetPageFromDisk(Page page, long pos)
        {
            var startAddress = pos - pos%_pageSize;
            page.StartAddress(startAddress);
            IoSeek(page._startAddress);
            var count = IoRead(page);
            if (count > 0)
            {
        ...
        {
            var count = _io.Read(page._buffer);
            if (count > 0)
            {
