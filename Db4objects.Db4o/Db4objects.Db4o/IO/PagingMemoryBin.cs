...
namespace Db4objects.Db4o.IO
{
    ...
    internal class PagingMemoryBin : IBin
    {
        ...
        {
            var avail = Length() - pos;
            if (avail <= 0)
            {
                return -1;
            }
            var bytesToRead = Math.Min((int) avail, length);
            var offset = PageOffset(pos);
            var pageIdx = PageIdx(pos);
            var bytesRead = 0;
            while (bytesRead < bytesToRead)
            {
                var curPage = ((byte[]) _pages[pageIdx]);
                var chunkLength = Math.Min(length - bytesRead, _pageSize - offset);
                Array.Copy(curPage, offset, buffer, bytesRead, chunkLength);
        ...
        public virtual void Write(long pos, byte[] buffer, int length)
        {
            ...
            {
                var curPage = ((byte[]) _pages[pageIdx]);
                var chunkLength = Math.Min(length - bytesWritten, _pageSize - offset);
                Array.Copy(buffer, bytesWritten, curPage, offset, chunkLength);
        ...
        private void EnsureLength(long length)
        {
            ...
            var lastPosPageIdx = PageIdx(lastPos);
            var lastPosPageLength = PageOffset(lastPos) + 1;
            ...
            for (var newPageIdx = _pages.Count; newPageIdx <= lastPosPageIdx; newPageIdx++)
            {
