...
namespace Db4objects.Db4o.Reflect.Core
{
    ...
    public abstract class AbstractReflectArray : IReflectArray
    {
        ...
        {
            var count = 0;
            var claxx = _reflector.ForObject(arr);
            while (claxx.IsArray())
            {
                count++;
            ...
            , object[] a_flat, int a_flatElement)
        {
            ...
            {
                for (var i = 0; i < a_dimensions[a_currentDimension]; i++)
                {
        ...
        {
            return Runtime.GetArrayValue(onArray, index);
        }
        ...
        {
            return Runtime.GetArrayLength(array);
        }
        ...
        public virtual void Set(object onArray, int index, object element)
        {
            if (element == null)
            {
                try
                {
                    Runtime.SetArrayValue(onArray, index, element);
                }
                catch (Exception)
                {
                }
