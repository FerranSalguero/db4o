...
namespace Db4objects.Db4o.Internal.Cluster
{
    ...
    {
        internal readonly Db4o.Cluster.Cluster _cluster;
        ...
        private ClusterConstraint Compatible(IConstraint
            with)
        {
            if (!(with is ClusterConstraint))
            {
                throw new ArgumentException();
            }
            var other = (ClusterConstraint
                ) with;
        ...
        private IConstraint Join(IConstraint with, bool isAnd)
        {
            lock (_cluster)
            {
                var other = Compatible(with);
                var newConstraints = new IConstraint[_constraints.Length];
                for (var i = 0; i < _constraints.Length; i++)
                {
                    newConstraints[i] = isAnd
                        ? _constraints[i].And(other._constraints[i])
                        : _constraints
