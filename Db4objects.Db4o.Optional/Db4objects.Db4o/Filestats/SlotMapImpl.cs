...
namespace Db4objects.Db4o.Filestats
{
    ...
    public class SlotMapImpl : ISlotMap
    {
        ...
        public virtual IList Merged()
        {
            ...
            var mergedSlot = ByRef.NewInstance(new Slot(0, 0));
        ...
        public virtual IList Gaps(long length)
        {
            ...
            var isFirst = true;
            Slot prevSlot = null;
            for (var curSlotIter = merged.GetEnumerator(); curSlotIter.MoveNext();)
            {
                var curSlot = ((Slot) curSlotIter.Current);
                if (isFirst)
                {
                    prevSlot = curSlot;
                ...
                {
                    var gapStart = prevSlot.Address() + prevSlot.Length();
                    gaps.Add(new Slot(gapStart, curSlot.Address() - gapStart));
                    prevSlot = curSlot;
                }
            }
            var afterlast = prevSlot.Address() + prevSlot.Length();
            if (afterlast < length)
            {
        ...
        {
            var totalLength = 0;
            for (var gapIter = slots.GetEnumerator(); gapIter.MoveNext();)
            {
                var gap = ((Slot) gapIter.Current);
        ...
        private sealed class _IVisitor4_32 : IVisitor4
        {
            ...
            {
                var curSlot = ((Slot) ((TreeIntObject) node)._object);
