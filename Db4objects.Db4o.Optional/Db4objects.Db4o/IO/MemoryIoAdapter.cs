...
namespace Db4objects.Db4o.IO
{
    ...
    {
        private readonly Hashtable4 _memoryFiles;
        private byte[] _bytes;
        private int _growBy;
        private int _length;
        ...
        {
            Array.Copy(_bytes, _seekPos, bytes, 0, length);
            _seekPos += length;
            return length;
        }
        ...
        public override void Write(byte[] buffer, int length)
        {
            ...
            {
                var growBy = _growBy;
                var missing = _seekPos + length - _bytes.Length;
                if (missing > growBy)
                {
                    growBy = missing;
                }
                var temp = new byte[_bytes.Length + growBy];
                Array.Copy(_bytes, 0, temp, 0, _length);
                _bytes = temp;
            }
            Array.Copy(buffer, 0, _bytes, _seekPos, length);
            _seekPos += length;
            if (_seekPos > _length)
            {
                _length = _seekPos;
