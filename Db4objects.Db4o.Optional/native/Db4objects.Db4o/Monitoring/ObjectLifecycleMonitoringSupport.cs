...
namespace Db4objects.Db4o.Monitoring
{
    public class ObjectLifecycleMonitoringSupport : IConfigurationItem
    {
        ...
        public void Apply(IInternalObjectContainer container)
        {
            ...
            EventHandler<ObjectInfoEventArgs> eventHandler = delegate { storedObjectsPerSec.Increment(); };
            eventRegistry.Created += eventHandler;
            eventRegistry.Updated += eventHandler;

            eventRegistry.Activated += delegate { activatedObjectsPerSec.Increment(); };
            eventRegistry.Deactivated += delegate { deactivatedObjectsPerSec.Increment(); };
            ...
            var deletedObjectsPerSec =
                Db4oPerformanceCounters.CounterFor(PerformanceCounterSpec.ObjectsDeletedPerSec, false);
            eventRegistry.Deleted += delegate { deletedObjectsPerSec.Increment(); };

            eventRegistry.Closing += delegate { deletedObjectsPerSec.Dispose(); };
        }
    }
}
