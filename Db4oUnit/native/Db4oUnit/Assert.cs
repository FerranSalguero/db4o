...
namespace Db4oUnit
{
    ...
    {
        public static Exception Expect(Type exception, CodeBlock block)
        {
            return Expect(exception, new DelegateCodeBlock(block));
        }

        public static Exception Expect<TException>(CodeBlock block) where TException : Exception
        {
            return Expect(typeof (TException), block);
        }

        public static void InRange(double value, double from, double to)
        {
            IsTrue(value >= from && value <= to, string.Format("'{0}' not in range '{1}'..'{2}'", value, from, to));
        }
