...
namespace Db4oUnit.Tests
{
    public class OpaqueTestSuiteTestCase : ITestCase
    {
        ...
        {
            AssertTestRuns(new[] {NumTests/2});
        }

        public virtual void TestAllFail()
        {
            var failingIndices = new int[NumTests];
        ...
        {
            var counter = new IntByRef();
            TestResult result = new _TestResult_29();
            new TestRunner(Iterators.Iterable(new[]
            {
                new SimpleTestSuite(counter, NumTests, failingIndices)
            }
                )).Run(result);
        ...
        private sealed class _TestResult_29 : TestResult
        {
            public override void TestStarted(ITest test)
            {
                ...
                Assert.IsInstanceOf(typeof (CountingTest), test);
            }
        }

        public class SimpleTestSuite : OpaqueTestSuiteBase
        {
            private readonly IntByRef _counter;
            ...
            private sealed class _IClosure4_47 : IClosure4
            {
                ...
                public object Run()
                {
                    return Iterators.Iterate(Tests(counter, numTests
                        , failingIndices));
                }
            }
        }
