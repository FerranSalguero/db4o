...
namespace Db4objects.Db4o.CS.Internal.Config
{
    public class ServerConfigurationImpl : NetworkingConfigurationProviderImpl, IServerConfiguration
    {
        ...
        {
            get { return new CacheConfigurationImpl(Legacy()); }
        }

        public virtual IFileConfiguration File
        {
            get { return Db4oLegacyConfigurationBridge.AsFileConfiguration(Legacy()); }
        }

        public virtual ICommonConfiguration Common
        {
            get { return Db4oLegacyConfigurationBridge.AsCommonConfiguration(Legacy()); }
        }

        public virtual int TimeoutServerSocket
        {
            get { return Legacy().TimeoutServerSocket(); }
            set
            {
        ...
        {
            get { return new IdSystemConfigurationImpl(Legacy()); }
        }
        ...
        public virtual void ApplyConfigurationItems(IObjectServer server)
        {
            ...
            for (var configItemIter = _configItems.GetEnumerator();
                configItemIter.MoveNext
                    ();)
            {
                var configItem = ((IServerConfigurationItem) configItemIter.Current
                    );
