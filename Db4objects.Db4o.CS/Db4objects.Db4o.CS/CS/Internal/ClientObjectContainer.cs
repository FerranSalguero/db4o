...
namespace Db4objects.Db4o.CS.Internal
{
    ...
    {
        private readonly BlockingQueue _asynchronousMessageQueue = new BlockingQueue();
        private readonly Collection4 _batchedMessages = new Collection4();
        ...
        private readonly BlockingQueue _synchronousMessageQueue = new BlockingQueue();
        ...
        private IMessageListener _messageListener = new _IMessageListener_87
            ();
        ...
        {
            var msg = (MsgBlob) Msg.ReadBlob.GetWriterForInt(trans, (int) GetID(blob));
            msg._blob = blob;
        ...
        {
            var msg = (MDeleteBlobFile) Msg.DeleteBlobFile.GetWriterForInt(trans,
                (int) GetID(blob));
            WriteMsg(msg, false);
        }
        ...
        public override Db4oDatabase Identity()
        {
            ...
            {
                Write(Msg.Identity);
                var reader = ExpectedBufferResponse(Msg.IdList);
        ...
        {
            ConfigImpl.PrefetchSettingsChanged += new _IEventListener4_145
                (this).OnEvent;
        ...
        private void StartDispatcherThread(Socket4Adapter socket, string user)
        {
            ...
            var dispatcherImpl = new ClientMessageDispatcherImpl(this
                , socket, _synchronousMessageQueue, _asynchronousMessageQueue);
            var dispatcherName = "db4o client side message dispatcher for " + user;
        ...
        public override sealed void Commit1(Transaction trans)
        {
            trans.Commit();
        }
        ...
        internal virtual Socket4Adapter CreateParallelSocket()
        {
            ...
            var serverThreadID = ExpectedBufferResponse(Msg.IdList).ReadInt();
            var sock = _socket.OpenParalellSocket();
            LoginToServer(sock);
            if (switchedToFile != null)
            {
                var message = Msg.SwitchToFile.GetWriterForString(SystemTransaction(), switchedToFile
                    );
                message.Write(sock);
                if (!(Msg.Ok.Equals(Msg.ReadMessage(this, SystemTransaction(), sock))))
                {
                    throw new IOException(Db4o.Internal.Messages.Get(42));
                }
            }
            ...
            , ClassMetadata superClazz)
        {
            ...
            var message = (MsgObject) resp;
            var bytes = message.Unmarshall();
            if (bytes == null)
            {
                return false;
            }
        ...
        {
            var classMeta = _classInfoHelper.GetClassMeta(reflectClass);
        ...
        public override sealed bool Delete4(Transaction ta, ObjectReference yo, object obj
            , int a_cascade, bool userCall)
        {
            var msg = Msg.Delete.GetWriterForInts(_transaction, new[]
            {
                yo.GetID(), userCall
                    ? 1
                    : 0
        ...
        {
            var msg = ExpectedResponse(expectedMessage);
            if (msg == null)
            {
        ...
        {
            var mode = Config().EvaluationMode().AsInt();
            var msg = Msg.GetAll.GetWriterForInts(trans, new[]
            {
                mode, PrefetchDepth(),
                PrefetchCount()
            });
        ...
        public override sealed HardObjectReference GetHardReferenceBySignature(Transaction
            trans, long uuid, byte[] signature)
        {
            var messageLength = Const4.LongLength + Const4.IntLength + signature.Length;
            var message = Msg.ObjectByUuid.GetWriterForLength(trans, messageLength);
        ...
        public virtual Msg GetResponse()
        {
            while (true)
            {
                var msg = _singleThreaded
                    ? GetResponseSingleThreaded()
        ...
        private Msg GetResponseSingleThreaded()
        {
            while (IsMessageDispatcherAlive())
            {
                try
                {
                    var message = Msg.ReadMessage(this, _transaction, _socket);
        ...
        public override ClassMetadata ClassMetadataForID(int clazzId)
        {
            ...
            var yc = base.ClassMetadataForID(clazzId);
            if (yc != null)
            {
                return yc;
            }
            var msg = Msg.ClassNameForId.GetWriterForInt(SystemTransaction(), clazzId);
            Write(msg);
            var message = (MsgD) ExpectedResponse(Msg.ClassNameForId);
            var className = Config().ResolveAliasStoredName(message.ReadString());
            if (className != null && className.Length > 0)
            {
                var claxx = Reflector().ForName(className);
                if (claxx != null)
                {
        ...
        {
            var stringWriter = new UnicodeStringIO();
            var length = stringWriter.Length(_userName) + stringWriter.Length(_password);
            var message = Msg.Login.GetWriterForLength(SystemTransaction(), length);
            message.WriteString(_userName);
            message.WriteString(_password);
            message.Write(iSocket);
            var msg = ReadLoginMessage(iSocket);
            ByteArrayBuffer payLoad = msg.PayLoad();
            BlockSize(payLoad.ReadInt());
            var doEncrypt = payLoad.ReadInt();
            if (doEncrypt == 0)
            {
        ...
        {
            var msg = Msg.ReadMessage(this, SystemTransaction(), iSocket);
        ...
        {
            var prefetchIDCount = Config().PrefetchIDCount();
            EnsureIDCacheAllocated(prefetchIDCount);
            ByteArrayBuffer reader = null;
            if (remainingIDs < 1)
            {
                var msg = Msg.PrefetchIds.GetWriterForInt(_transaction, prefetchIDCount);
                Write(msg);
                reader = ExpectedBufferResponse(Msg.IdList);
                for (var i = prefetchIDCount - 1; i >= 0; i--)
                {
        ...
        internal virtual void ProcessBlobMessage(MsgBlob msg)
        {
            lock (_blobLock)
            {
                var needStart = _blobTask == null || _blobTask.IsTerminated();
                if (needStart)
                {
        ...
        {
            var msg = Msg.ReadSlot.GetWriterForInts(_transaction, new[]
            {
                a_address, a_length
            });
            Write(msg);
            var reader = ExpectedBufferResponse(Msg.ReadSlot);
            Array.Copy(reader._buffer, 0, a_bytes, 0, a_length);
        }
        ...
        public override sealed StatefulBuffer ReadStatefulBufferById(Transaction a_ta, int
            a_id)
        {
        ...
        {
            var msg = Msg.ReadObject.GetWriterForInts(a_ta, new[]
            {
                a_id, lastCommitted
                    ? 1
                    : 0
            ...
            Write(msg);
            var bytes = ((MsgObject) ExpectedResponse(Msg.ObjectToClient)).Unmarshall
                ();
            if (bytes != null)
            {
        ...
        public override sealed ByteArrayBuffer[] ReadSlotBuffers(Transaction transaction,
            int[] ids)
        {
        ...
        {
            var prefetchDepth = Config().PrefetchDepth();
            ...
            prefetchDepth)
        {
            ...
            var cacheMisses = PopulateSlotBuffersFromCache(transaction, ids, buffers);
            FetchMissingSlotBuffers(transaction, cacheMisses, buffers, prefetchDepth);
            return PackSlotBuffers(ids, buffers);
        }

        public override sealed ByteArrayBuffer ReadBufferById(Transaction transaction, int
            id, bool lastCommitted)
        {
            ...
            var cached = _clientSlotCache.Get(transaction, id);
            if (cached != null)
            {
                return cached;
            }
            var slot = FetchSlotBuffer(transaction, id, lastCommitted);
            _clientSlotCache.Add(transaction, id, slot);
            return slot;
        }

        public override sealed ByteArrayBuffer ReadBufferById(Transaction a_ta, int a_id)
        {
        ...
        internal virtual void ReadThis()
        {
            ...
            var bytes = ExpectedBufferResponse(Msg.GetClasses);
            ClassCollection().SetID(bytes.ReadInt());
            var stringEncoding = bytes.ReadByte();
        ...
        public override bool SetSemaphore(Transaction trans, string name, int timeout)
        {
            lock (_lock)
            {
                ...
                var msg = Msg.SetSemaphore.GetWriterForIntString(trans, timeout, name);
                Write(msg);
                var message = GetResponse();
        ...
        {
            var msg = Msg.WriteNew.GetWriter(trans, pointer, classMetadata, buffer);
            WriteBatchedMessage(msg);
        }
        ...
        {
            var msg = Msg.WriteUpdate.GetWriter(trans, pointer, classMetadata, arrayType.Value
                (), buffer);
            WriteBatchedMessage(msg);
        }
        ...
        private void EnsureIDCacheAllocated(int prefetchIDCount)
        {
            ...
            {
                var newPrefetchedIDs = new int[prefetchIDCount];
                Array.Copy(_prefetchedIDs, 0, newPrefetchedIDs, 0, _prefetchedIDs.Length);
        ...
        {
            var msg = Msg.GetInternalIds.GetWriterForInts(trans, new[]
            {
                clazz.GetID(),
        ...
        {
            var trans = query.Transaction();
            var ids = GetIDsForClass(trans, clazz, true);
            var resClient = new ClientQueryResult(trans, ids.Length);
        ...
        {
            var ids = new long[idIterator.Size()];
            var i = 0;
        ...
        public override IQueryResult ExecuteQuery(QQuery query)
        {
            ...
            query.Marshall();
            var msg = Msg.QueryExecute.GetWriter(Serializer.Marshall(trans, query));
            Write(msg);
            return ReadQueryResult(trans);
        }
        ...
        public void WriteBatchedMessages()
        {
            lock (Lock())
            {
                ...
                Msg msg;
                var multibytes = Msg.WriteBatchedMessages.GetWriterForLength(Transaction, _batchedQueueLength
                    );
                multibytes.WriteInt(_batchedMessages.Size());
                var iter = _batchedMessages.GetEnumerator();
                while (iter.MoveNext())
                {
        ...
        {
            var classMetadata = base.ClassMetadataForReflectClass(claxx);
            if (classMetadata != null)
            {
                return classMetadata;
            }
            var className = Config().ResolveAliasRuntimeName(claxx.GetName());
        ...
        {
            var msg = Msg.ClassMetadataIdForName.GetWriterForString(SystemTransaction(), name
                );
            msg.Write(_socket);
            var response = (MsgD) ExpectedResponse(Msg.ClassId);
            return response.ReadInt();
        }

        public override int InstanceCount(ClassMetadata clazz, Transaction trans)
        {
            var msg = Msg.InstanceCount.GetWriterForInt(trans, clazz.GetID());
            Write(msg);
            var response = (MsgD) ExpectedResponse(Msg.InstanceCount);
            return response.ReadInt();
        }

        public virtual void MessageListener(IMessageListener listener
            )
        {
        ...
        {
            var configuredBatchMessages = _config.BatchMessages();
        ...
        {
            var msg = message.GetWriterForLength(transaction, Const4.IntLength + Const4.IntLength
                                                              + Const4.IdLength*idsToRead.Count);
            msg.WriteInt(prefetchDepth);
            msg.WriteInt(idsToRead.Count);
            for (var idIter = idsToRead.GetEnumerator(); idIter.MoveNext();)
            {
            ...
            IDictionary buffers, int prefetchDepth)
        {
            ...
            var safePrefetchDepth = Math.Max(1, prefetchDepth);
            SendReadMultipleObjectsMessage(Msg.ReadMultipleObjects, transaction, safePrefetchDepth
                , missing);
            var response = (MsgD) ExpectedResponse(Msg.ReadMultipleObjects);
            var slots = new CacheContributingObjectReader((ClientTransaction) transaction
                , _clientSlotCache, response.PayLoad()).Buffers();
            while (slots.MoveNext())
            {
                var pair = ((Pair) slots.Current);
        ...
        {
            var returnValue = new ByteArrayBuffer[buffers.Count];
            for (var i = 0; i < ids.Length; ++i)
            {
            ...
            , IDictionary buffers)
        {
            ...
            {
                var id = ids[idIndex];
                var slot = _clientSlotCache.Get(transaction, id);
                if (null == slot)
                {
                    missing.Add(id);
                }
        ...
        {
            var msg = Msg.ReadReaderById.GetWriterForInts(transaction, new[]
            {
                id, lastCommitted
                    ? 1
                    : 0
            ...
            Write(msg);
            var buffer = ((MReadBytes) ExpectedResponse(Msg.ReadBytes)).Unmarshall
                ();
            return buffer;
        }
        ...
        public override bool IsDeleted(Transaction trans, int id)
        {
            ...
            var msg = Msg.TaIsDeleted.GetWriterForInt(trans, id);
            Write(msg);
            var res = ExpectedBufferResponse(Msg.TaIsDeleted).ReadInt();
            return res == 1;
        }
        ...
        public virtual void CommitReplication(long replicationRecordId, long timestamp)
        {
            ...
            {
                CheckReadOnly();
                var clientTransaction = (ClientTransaction) Transaction;
                clientTransaction.PreCommit();
                Write(Msg.CommitReplication.GetWriterForLongs(clientTransaction, new[]
                {
                    replicationRecordId
                    , timestamp
                }));
        ...
        private sealed class _IMessageListener_87 : IMessageListener
        {
        ...
        private sealed class _IEventListener4_145
        {
            ...
            {
                _enclosing.InitalizeClientSlotCache();
            }
        }
        ...
        private sealed class _IRunnable_670 : IRunnable
        {
            ...
            public void Run()
            {
                var reader = _enclosing.ExpectedBufferResponse(Msg.QueryResult);
                var queryResultID = reader.ReadInt();
                var queryResult = _enclosing.QueryResultFor(trans, queryResultID
                    );
                queryResult.LoadFromIdReader(_enclosing.IdIteratorFor(trans, reader));
        ...
        private sealed class _IRunnable_901 : IRunnable
        {
            ...
            public void Run()
            {
                var reader = _enclosing.ExpectedBufferResponse(Msg.IdList);
                var idIterator = _enclosing.IdIteratorFor(trans, reader);
                result.value = _enclosing.ToLongArray(idIterator);
            }
        }
    }
}