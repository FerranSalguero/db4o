    ...
    public class MCommitReplication : MCommit, IMessageWithResponse
    {
        public override Msg ReplyFromServer()
        {
            ...
            lock (ContainerLock())
            {
                ...
                var replicationRecordId = ReadLong();
                var timestamp = ReadLong();
                var concurrentTimestamps = trans.ConcurrentReplicationTimestamps();
                ServerMessageDispatcher().Server().BroadcastReplicationCommit(timestamp, concurrentTimestamps
                    );
                var replicationRecord = (ReplicationRecord) Container().GetByID(trans
                    , replicationRecordId);
