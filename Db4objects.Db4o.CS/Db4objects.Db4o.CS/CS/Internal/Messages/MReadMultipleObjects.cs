...
namespace Db4objects.Db4o.CS.Internal.Messages
{
    public class MReadMultipleObjects : MsgD, IMessageWithResponse
    {
        public Msg ReplyFromServer()
        {
            ...
            var buffer = MarshallObjects(prefetchDepth, prefetchCount, ids);
            return ReadMultipleObjects.GetWriterForBuffer(Transaction(), buffer);
        }
            ...
            ids)
        {
            lock (ContainerLock())
            {
                var strategy = ObjectExchangeStrategyFactory.ForConfig(new ObjectExchangeConfiguration
