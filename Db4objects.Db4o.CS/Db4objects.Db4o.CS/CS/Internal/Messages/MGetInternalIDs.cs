...
namespace Db4objects.Db4o.CS.Internal.Messages
{
    public sealed class MGetInternalIDs : MsgD, IMessageWithResponse
    {
            ...
            prefetchCount, bool triggerQueryEvents)
        {
            lock (ContainerLock())
            {
                var ids = GetIDs(classMetadataID, triggerQueryEvents);
        ...
        private long[] GetIDs(int classMetadataID, bool triggerQueryEvents)
        {
            lock (ContainerLock())
            {
                var classMetadata = Container().ClassMetadataForID(classMetadataID);
        ...
        private sealed class _IClosure4_45 : IClosure4
        {
            ...
            public object Run()
            {
                return classMetadata.GetIDs(_enclosing.Transaction());
            }
        }
    }
}