...
namespace Db4objects.Db4o.CS.Internal.Messages
{
    ...
    public class MObjectSetFetch : MObjectSet, IMessageWithResponse
    {
        ...
        {
            var queryResultID = ReadInt();
            var fetchSize = ReadInt();
            var fetchDepth = ReadInt();
            MsgD message = null;
            lock (ContainerLock())
            {
                var idIterator = Stub(queryResultID).IdIterator();
                var payload = ObjectExchangeStrategyFactory.ForConfig(new ObjectExchangeConfiguration
