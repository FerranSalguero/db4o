...
namespace Db4objects.Db4o.CS.Internal.Messages
{
    public class MsgObject : MsgD
    {
        ...
        internal MsgD GetWriter(Transaction trans, Pointer4 pointer, ByteArrayBuffer buffer
            , int[] prependInts)
        {
            var lengthNeeded = buffer.Length() + LengthForFirst;
            if (prependInts != null)
            {
                lengthNeeded += (prependInts.Length*Const4.IntLength);
            }
            var message = GetWriterForLength(trans, lengthNeeded);
            if (prependInts != null)
            {
                for (var i = 0; i < prependInts.Length; i++)
                {
        ...
        public override sealed MsgD GetWriter(StatefulBuffer buffer)
        {
            ...
            , ByteArrayBuffer buffer)
        {
            ...
            {
                return GetWriter(trans, pointer, buffer, new[] {0});
            }
            return GetWriter(trans, pointer, buffer, new[] {classMetadata.GetID()});
        }
