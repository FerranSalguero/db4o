...
namespace Db4objects.Db4o.CS.Internal.Messages
{
    public sealed class MWriteUpdate : MsgObject, IServerSideMessage
    {
        public void ProcessAtServer()
        {
            ...
            var arrayTypeValue = _payLoad.ReadInt();
            var arrayType = ArrayType.ForValue(arrayTypeValue);
            Unmarshall(_payLoad._offset);
            lock (ContainerLock())
            {
                var classMetadata = LocalContainer().ClassMetadataForID(classMetadataID
                    );
                var id = _payLoad.GetID();
                Transaction().DontDelete(id);
                var clientSlot = _payLoad.Slot();
                Slot newSlot = null;
        ...
        {
            var reference = Transaction().ReferenceForId(id);
            if (null == reference)
            {
                return;
            }
